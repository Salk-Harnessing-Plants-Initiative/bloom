...
- **bloommcp data directories**: the `bloommcp` container's non-root `bloom` user needs write access to the three host directories bind-mounted at `bloommcp/data/{TRAITS_DIR,PLOTS_DIR,ANALYSIS_OUTPUT}`. On a fresh clone none of these exist, so Docker auto-creates them owned by the daemon's user (root) — unwritable by the container. `make dev-up`'s `ensure-bloommcp-data-dirs` preflight (`scripts/ensure_bloommcp_data_dirs.sh`) creates and `chmod 777`s them before `docker compose up` runs; see `openspec/changes/archive/2026-08-09-fix-bloommcp-dev-data-dir-permissions/design.md` for why `777` (not UID-matching or named volumes) was chosen for these specific, non-secret, gitignored local-scratch directories. This preflight runs unconditionally — deliberately not folded into the `doctor` preflight, which CI's dev-stack-smoke job skips via `DOCTOR_SKIP=1`. The same preflight also runs on staging and production before `docker compose up` (`deploy.yml`'s `deploy-staging`/`deploy-production` jobs) and in this repo's own `compose-health-check` CI job (`pr-checks.yml`) — see `openspec/changes/add-bloommcp-prod-staging-data-dir-preflight/design.md` for the staging/production-specific reasoning, including why `chmod 777` (not UID/GID matching) is used there too: this is a narrow, explicitly-documented exception to this constraint's own general MinIO-derived guidance above, justified because the `bloom` user's UID is assigned arbitrarily at Docker image-build time (not fixed/predictable), so UID/GID matching isn't achievable without a separate `bloommcp/Dockerfile` change. Note:
  "non-secret, gitignored local-scratch" describes the _confidentiality_ profile of all three
  directories (true in every environment) — it does NOT mean disposable-scratch everywhere.
  On staging/production specifically, `TRAITS_DIR` is `qc_clean`'s sole copy of raw experiment
  input with no Supabase Storage backup (see `rename-bloommcp-sleap-out-csv-dir`'s
  investigation); losing or corrupting it there is real data loss, not cache invalidation. The
  `chmod 777` exception is still accepted for that reason (no secrets, `single-tenant`-host
  local-process exposure only) — just don't cite "local-scratch" as precedent for widening
  permissions on something where sole-copy data loss would matter even more.
