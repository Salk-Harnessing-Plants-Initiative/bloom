# bloommcp-get-download-links-tool Specification

## Purpose
Defines the `get_download_links` MCP tool: how it is registered and discovered, and the guarantee
that it returns the result store's freshly re-signed download links **unmodified**. The tool exists
so a caller can retrieve usable links for a previously persisted analysis run without re-running
it. Re-signing happens in the result store; this tool must not rewrite, truncate, or re-derive what
that layer returns.
## Requirements
