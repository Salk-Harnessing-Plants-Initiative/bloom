# bloommcp-viz-tools Specification

## Purpose
Covers the contract-wrapped trait visualization tools (correlation matrix, histograms, boxplots and
their siblings) as a single converged surface rather than per-tool one-offs.

Beyond registration and persistence, it specifies the properties that keep a rendered figure honest
about the data behind it: zero-variance traits and low-overlap trait pairs are excluded **and
disclosed** rather than silently dropped, a masking mismatch between the computed matrix and the
rendered heatmap is reported, and the resolved trait selection is recorded rather than merely
counted. It also covers figure-registry concurrency safety, the path-safety guard that runs before
any read, and versioned run persistence under per-tool tool classes.
## Requirements
