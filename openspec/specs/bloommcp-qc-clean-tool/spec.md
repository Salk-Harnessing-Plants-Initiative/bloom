# bloommcp-qc-clean-tool Specification

## Purpose
Bloom MCP Tier 3 QC tool. Exposes a `qc_clean` MCP tool that turns a raw experiment trait
table into a no-NaN cleaned CSV by delegating to `sleap_roots_analyze.clean_traits_for_analysis`
— the tool contains no QC logic of its own. Persists the cleaned run as a versioned artifact via
the `ResultStore` port (tool class `qc`) so downstream tools (e.g. `pca_analysis`) can resolve
it via `require_clean=True`. Establishes the producer side of the tool-composition pattern and
proves the contract + persistence write path end-to-end (Tier 3, issue #338).
## Requirements
...
- **WHEN** `QCCleanParams` is constructed with no threshold overrides
- **THEN** `max_zeros_per_trait == 0.5`, `max_nans_per_trait == 0.2`,
  `max_nans_per_sample == 0.0`, and `min_samples_per_trait == 10` — the canonical
  `CleanupConfig` values, not the delegate helper's looser `0.3` / `0.2`
