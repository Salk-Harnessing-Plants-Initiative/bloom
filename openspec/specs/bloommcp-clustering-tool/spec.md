# bloommcp-clustering-tool Specification

## Purpose
Covers the plotting surface of bloommcp's clustering tool: which plots a caller may request, and
what the tool guarantees about producing them. Plot keys are validated **before** any part of the
run is committed, so an unrecognised key fails the request outright rather than leaving a
half-persisted run behind. Generated PNGs are persisted into the run and returned as object-key
links rather than inline bytes. Generation delegates entirely to the upstream plotters, uniformly
across clustering methods — the tool chooses *what* to plot, never how to draw it, so a plot's
appearance stays a property of the upstream library rather than something this layer can drift
from.
## Requirements
