# cyl-test-scan-cli Specification

## Purpose
Defines `bloomctl cyl create-test-scan`, the committed way to mint synthetic cylinder scans in the
staging test experiment — replacing the throwaway scripts that had been written and discarded
repeatedly, and unblocking every verification that needs a scan with **no prior pipeline-run
envelope** (an already-ingested scan makes a re-delivery an idempotent no-op, which silently
invalidates the result).

Two modes: `--poison` creates a deliberately undownloadable scan, reproducing the "1 of 1 frames
failed to download" failure that partial-success verification depends on; `--good` creates a scan
with real, downloadable imagery through the same insert → upload → update path Bloom Desktop uses.

Most of this capability is safety, because the command mutates a shared staging database. It is
hardcoded to one experiment and **verifies that experiment live by name** before any write — not by
id alone, and not by the name prefix, since `insert_image_v2_0` upserts on an exact
`(species_id, name)` match and a prefix would silently create a stray experiment instead of
attaching to the intended one. Invocations are serialized by a same-machine file lock, because the
RPC's `ON CONFLICT ... DO NOTHING` upserts never raise on a QR-suffix race and two concurrent runs
would quietly merge into one shared scan. A pre-existing SUCCESS row is a hard failure rather than a
silent reuse, identity fields use fixed synthetic sentinels so no real staff or accession data is
fabricated, and abandoned scans are surfaced on every invocation rather than left to accumulate
undetected.
## Requirements
