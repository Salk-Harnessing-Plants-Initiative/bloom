...
- [ ] 1.1 Add direct unit tests for the primitives *in their new home*, mirroring how `#585`
...
- [ ] 1.2 Move `_UNTRUSTWORTHY_FIT` (rename `UNTRUSTWORTHY_FIT_QUALITIES`) and
...
- [ ] 1.3 Update `remove_outliers.py` to import all three from `experiment_utils` instead of
      defining them; remove the now-dead private definitions.
- [ ] 1.4 Add a symbol-relocation regression test (same pattern as `#403`'s
...
- [ ] 1.5 Re-run `bloommcp/tests/tools/test_remove_outliers_tool.py` unmodified — a pure
...
- [ ] 2.1 Add `write_outlier_trim_manifest(tmp_path, stem, tool_class, version_id, created_at,
...
- [ ] 3.1 Write failing tests for `scan_for_untrustworthy_outlier_fits()` in a new
...
- [ ] 3.2 Implement `bloommcp/scripts/audit_untrustworthy_outlier_fits.py`:
...
- [ ] 3.3 Write failing tests for `write_report()`'s collision-avoidance (mirrors `#585`'s
...
- [ ] 3.4 Implement `write_report(report) -> str` — same shape as `#585`'s (`scanned_at`,
      ...
      3.3's tests pass.
- [ ] 3.5 Write failing tests for `run()`/`main()`: a successful scan returns `0`, prints the
...
- [ ] 3.6 Implement `run()`/`main()` — same exit-code contract as `#585`'s script. Confirm 3.5's
      tests pass.
- [ ] 3.7 Write the script's module docstring — purpose, read-only disclosure, the `scope_note`'s
...
- [ ] 4.1 `openspec validate add-bloommcp-outliers-fit-audit --strict` passes.
- [ ] 4.2 Full `bloommcp` suite green (`uv run --extra test pytest -q -m 'not live_smoke'`), not
      just the new/changed files.
- [ ] 4.3 `ruff format --check` / `black --check` clean on all changed/new Python files.
