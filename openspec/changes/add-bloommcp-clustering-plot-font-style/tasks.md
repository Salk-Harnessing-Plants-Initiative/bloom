...
- [ ] 1.0 `test_plot_font_style_fields_exist` — `assert {"plot_font_family", "plot_font_size"} <=
...
- [ ] 1.1 `test_plot_font_family_and_size_forwarded_and_applied` — fake `_clustering_plot_calls`
...
- [ ] 1.2 `test_plot_font_style_applies_for_every_method` — parametrize over `_METHOD_CALLS`
...
- [ ] 1.3 `test_plot_font_size_out_of_range_is_invalid_input_regardless_of_include_plots` —
...
- [ ] 1.4 `test_plot_font_size_rejected_before_the_experiment_is_read` — **spy, do not raise.**
...
- [ ] 1.5 `test_plot_font_size_at_ceiling_is_accepted` — `plot_font_size=100` with a captured real
...
- [ ] 1.6 `test_plot_font_size_just_above_zero_is_accepted` — `_run(include_plots=False,
...
- [ ] 1.7 `test_plot_font_fields_ignored_when_include_plots_false` — `_run(include_plots=False,
  plot_font_family="serif", plot_font_size=22)`; assert success and no PNG keys in `outputs`.
- [ ] 1.8 `test_unresolvable_plot_font_family_is_not_rejected` — parametrize over
  ...
  repo.
- [ ] 1.9 `test_plot_font_size_ceiling_is_in_the_json_schema` — assert
...
- [ ] 1.10 `test_method_control_conflict_is_reported_before_the_font_size_ceiling` — reuse
...
- [ ] 1.10b `test_font_size_ceiling_is_reported_before_plot_key_validation` — `_run(method=
...
- [ ] 1.11 Run the new tests and confirm the expected split — **measured against `HEAD`, do not
  assume all are red.** RED before implementation: 1.0, 1.1, 1.2, 1.3, 1.4, 1.5, 1.10b, 1.12,
  1.13, and 1.9's first assertion. GREEN before implementation **by construction, not by
  mistake**: 1.6, 1.7, 1.8, 1.10 and 1.9's `plot_font_size=99999` assertion — Pydantic's
  `extra="ignore"` drops the unknown field, so the no-error/no-PNG outcome is identical pre- and
  post-change. Those five are regression pins gated by 1.0; do not "fix" them by asserting a
  Pydantic rejection that `extra="ignore"` will never produce.
- [ ] 1.12 `test_default_call_leaves_upstream_plotter_styling_untouched` — same fake harness, but
  ...
  4.2.
- [ ] 1.13 `test_font_style_fields_are_recorded_in_provenance_params` — spy on `store.create_run`
...
- [ ] 2.1 `clustering.py` — widen the `_plots` import to
...
- [ ] 2.2 `clustering.py` — add `plot_font_family: str | None = Field(default=None, ...)` and
...
- [ ] 2.5 `clustering.py` — forward `font_family=params.plot_font_family,
...
- [ ] 2.6 `clustering.py` — add an "**Optional font-style override (#680).**" paragraph to the
  ...
  fallback.
- [ ] 2.7 `bloommcp/docs/roadmap.md` — add one line recording the audit's two untracked gaps
  ...
  the roadmap doesn't.
- [ ] 2.8 Re-run the tests from section 1; all green.

## 3. Verification

- [ ] 3.1 `uv run --extra test pytest tests/tools/test_clustering_tool.py -q` from `bloommcp/` —
  full file green, no pre-existing test modified.
- [ ] 3.2 `uv run --extra test pytest tests/tools/ -q` from `bloommcp/` — no collateral damage to
  `test_pca_analysis_tool.py`, `test_umap_analysis_tool.py`, or `test_plots_helpers.py`.
- [ ] 3.3 `uv run black --check src tests && uv run ruff check src tests` from `bloommcp/`.
- [ ] 3.4 `openspec validate add-bloommcp-clustering-plot-font-style --strict`.
- [ ] 3.5 Commit safety: the test commit may land red — `pr-checks.yml` triggers on
...
- [ ] 4.1 Diff `ClusteringParams`'s two new fields against `PCAAnalysisParams`'s — confirm the
...
- [ ] 4.2 Confirm the default path is untouched: with both fields `None`, `apply_font_style`
  ...
  pinned by 1.13.)
- [ ] 4.3 **Confirm the diff makes no behavior change to
  `bloommcp/src/bloom_mcp/tools/_plots.py`.** If it does, the change's central premise is wrong —
  stop and re-read `design.md`.
- [ ] 4.4 Confirm no other tool file is touched — in particular `heritability_analysis.py`, whose
