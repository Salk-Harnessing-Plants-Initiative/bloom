...
**Timeout value — revised after a second review round.** An initial version of this decision set the timeout at 45s ("comfortably above the 30s `start_period` plus one 10s `interval` cycle"). Review correctly pointed out this doesn't match Docker's actual healthcheck semantics: failures *during* `start_period` never count toward `retries`, and only after `start_period` elapses does Docker need `retries` (5) *consecutive* failing checks — up to `interval * retries` = `10s * 5` = 50s more — before it would ever call Kong `unhealthy`. So a Kong that is merely slow (not crashed), perhaps booting under load, can legitimately still be `starting` for up to `30 + 50 = 80s` without anything being wrong. A 45s bound would misclassify that as a timeout and hand off to the crash-loop check while Kong is still not actually ready — relocating the original race to the smoke-test step instead of closing it. **The timeout is now `90s`** (`start_period` + `interval * retries`, rounded up for scheduling slack) — matching Kong's own healthcheck's full worst-case-but-still-fine tolerance, so a timeout at this bound is a genuine signal something is wrong, not normal variance.
