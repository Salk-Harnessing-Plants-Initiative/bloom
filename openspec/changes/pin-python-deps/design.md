...
- **`COPY --from=ghcr.io/astral-sh/uv:0.11` in Docker**: Pin uv to latest stable minor (0.11). Multi-stage copy of the uv binary.
...
2. Update Dockerfiles to use uv with `UV_PROJECT_ENVIRONMENT=/opt/venv` (outside bind-mount path)
...
5. Update Makefile to use `uv run --with`
6. Update `dependabot.yml` to use `uv` ecosystem
7. Update `.claude/commands/` files with new audit commands
8. Delete `requirements.txt` files
9. Rollback: if uv sync fails in Docker, revert Dockerfile changes and restore requirements.txt (both are in git history)
