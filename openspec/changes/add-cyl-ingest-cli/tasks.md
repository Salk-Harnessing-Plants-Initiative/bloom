...
- [ ] 1.1 Author `proposal.md`, `design.md`, and the `cyl-ingest-cli` spec delta. Run
...
- [ ] 2.1 Add `sleap-roots-contracts>=0.1.0a3` to `bloomcli/pyproject.toml` dependencies (no
...
- [ ] 2.2 Register an `integration` pytest marker in `bloomcli/pyproject.toml`
  (`[tool.pytest.ini_options] markers = ["integration: requires live staging (env-gated)"]`).
- [ ] 2.3 Commit a real emitter-generated fixture at
...
- [ ] 2.4 Add `-m "not integration"` to **both** bloomcli test invocations so the default gate
...
- [ ] 3.1 `load_envelope`: parses a valid envelope from a **path**; reads from **stdin** when the
  source is `-`; raises a readable error on a **missing file**, **invalid JSON**, and **empty
  stdin**.
- [ ] 3.2 `validate_envelope`: **accepts** the committed real fixture; **rejects** a *targeted*
  ...
  dump.
- [ ] 3.3 send-original: the object handed to the RPC equals the originally parsed JSON (same
  `provenance.idempotency_key`; **no** `model_dump` substitution).
- [ ] 3.4 `summarize_result`: `was_noop=false` → an "ingested" summary naming `source_id` and the
...
- [ ] 3.5 `map_rpc_error` matches on the **exact** RPC substrings from
...
- [ ] 3.6 `map_rpc_error` is **exhaustive** over the 17 RPC `RAISE EXCEPTION` strings — the
...
- [ ] 3.7 validation-stricter-than-RPC: an envelope the RPC would accept but the model rejects
...
- [ ] 4.3 CLI no-op: RPC result `was_noop=true` (null `scan_id`) → exit 0, "already ingested"
  (not an error).
- [ ] 4.4 CLI no-scan: `call_insert_envelope` raises a **genuine** `postgrest.APIError({"message":
  "unresolvable image_ids: matched 1 of 2 to a scan", "code": "P0001"})` → non-zero exit with the
  actionable message.
- [ ] 4.5 CLI validation-fail-before-call: a malformed envelope → non-zero exit and
  `call_insert_envelope` / `_authed_client` are **never** invoked.
- [ ] 4.6 CLI `--json`: prints the RPC result object (parseable, includes `source_id` +
  `was_noop`) to stdout; without `--json`, stdout is a human summary line.
- [ ] 4.7 CLI auth error: missing credentials → guidance to run `bloomctl login`, non-zero exit.
- [ ] 4.8 CLI permission-denied: `call_insert_envelope` raises `postgrest.APIError({"message":
...
- [ ] 4.9 CLI blobs pass-through: an envelope with a **non-empty** `blobs` array → the dict
...
- [ ] 5.1 Add `bloomcli/src/bloomctl/ingest.py`: pure helpers (`load_envelope`,
...
- [ ] 6.1 `@pytest.mark.integration` module that self-skips at **collection/import** time when
...
- [ ] 9.1 `openspec validate add-cyl-ingest-cli --strict` passes.
- [ ] 9.2 bloomcli unit suite green **using the CI invocation** (`cd bloomcli && uv run --extra
