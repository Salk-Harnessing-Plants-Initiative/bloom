...
| Caddy migration from third-party image to GHCR custom build | **Filed in §12.10 + §9.5 forward-compat inlined** — depends on whether branch [`chore/caddy-acme-dns-cloudflare`](https://github.com/Salk-Harnessing-Plants-Initiative/bloom/tree/chore/caddy-acme-dns-cloudflare) merges before/after PR-3. If before, §9.5's inline checklist is followed during PR-3 implementation; if after, §12.10's issue tracks the standalone follow-up. Caddy is currently in the third-party-pin list (§9.2) and stays there until that branch lands a custom `caddy/Dockerfile`. |
