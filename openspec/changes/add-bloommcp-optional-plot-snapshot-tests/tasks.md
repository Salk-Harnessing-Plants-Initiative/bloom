...
- [x] 1.1 **RED** — add `test_optional_plot_keys_render_and_commit` to
...
- [x] 1.2 **GREEN** — add `_render_optional_to_dir(...)` next to `_render_to_dir`, differing
...
- [x] 1.3 **RED→GREEN** — `test_baseline_set_matches_the_tool_catalogs`: compare the on-disk
...
- [x] 2.1 Extend `scripts/gen_plot_snapshots_golden.py` with an `_OPTIONAL_TOOLS` table and
...
- [x] 2.2 Extend `write_manifest()` to record what these renders actually depend on:
...
- [x] 2.3 Run the generator with **no** `--yes` first; confirm it reports 8 "new baseline"
  lines and 3 unchanged RMS≈0 lines for the existing ones. Then rerun with `--yes`. Paste
  both outputs into the PR body.
- [x] 2.4 **Inspect all 8 PNGs before committing them.** A baseline generated from today's
...
- [x] 2.5 Record the known upstream defect this inspection surfaces:
...
- [x] 2.6 Confirm each committed PNG is under the 500 KB `check-added-large-files` limit
  (measured: largest 214.1 KB, total 580.6 KB) and that `git check-attr` reports `binary` via
  `.gitattributes`' `*.png` rule.
- [x] 2.7 Verify the 3 pre-existing baselines are byte-identical after the run — `git status`
  must show them unmodified. A change there means this PR altered coverage it does not own.
- [x] 2.8 **Only now** extend `tests/scripts/test_gen_plot_snapshots_golden.py` to cover the
...
- [x] 3.1 **RED** — `test_optional_plot_matches_baseline_within_tolerance`, parametrized over
...
- [x] 3.2 **GREEN** — with baselines committed from §2, all 8 pass.
- [x] 3.3 **RED→GREEN** — `test_missing_optional_baseline_fails_with_an_actionable_message`:
...
- [x] 3.4 **RED→GREEN** — `test_tolerance_catches_a_real_parameter_regression`, parametrized
...
- [x] 3.5 **RED→GREEN** — three cheap boundary tests for `_compare_or_fail`, which 8 more
...
- [x] 4.1 **RED→GREEN** — `test_cluster_scatter_pca_does_not_catch_a_cluster_count_change`.
...
- [x] 4.2 **RED→GREEN** — `test_cluster_size_barplot_does_not_catch_a_same_k_membership_change`:
...
- [x] 4.3 Confirm no negative control is written for
...
- [x] 5.1 Update `test_viz_snapshot.py`'s module docstring: its "Scope" paragraph currently
...
- [x] 5.2 Update `tests/fixtures/README.md`'s `plot_baselines/` section: the 8 new baselines,
  ...
  already described).
- [x] 5.3 Document in the README what the baselines encode implicitly, so a future failure is
...
- [x] 6.1 `openspec validate --strict` across the repo, not just this change — §7 edits a
  sibling change's delta.
- [x] 6.2 Full bloommcp sweep as CI runs it:
...
- [x] 6.3 Record the suite's before/after runtime in design.md (baseline measured: 190.0s,
...
- [x] 6.4 `black --check` + `ruff check` on the touched Python files.
- [x] 6.5 Confirm the new tests are unmarked, so `bloommcp-plot-snapshot-testing`'s existing
  "Runs Unmarked In Per-PR CI" requirement continues to hold for them.

## 7. Correct the sibling change's stale spec

- [x] 7.1 Amend `openspec/changes/add-bloommcp-plot-snapshot-tests/specs/bloommcp-plot-snapshot-testing/spec.md`:
...
- [x] 7.2 Re-run `openspec validate add-bloommcp-plot-snapshot-tests --strict` after the edit.
