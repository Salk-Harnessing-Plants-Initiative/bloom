...
- **Decision 2 — Downgrade *both* `logger.exception` call sites in `usage.py`, not only
  ...
  `logger.warning` with just the exception message.
- **Decision 3 — Reword the doc claim to "no experiment data leaves your machine" rather than
  ...
  offending network *attempt* in local mode, so after this change local mode again makes no
...
- **Decision 5 — The MODIFIED `bloommcp_usage Records Caller Activity Per Mounted Surface`
  ...
  of this proposal pasting the *original* (pre-OAuth) requirement text as its MODIFIED baseline.
  ...
  `add-bloommcp-oauth-usage-attribution` *after* this one, without re-checking their deltas
...
- Downgrading `logger.exception` → `logger.warning` for the RPC-failure path loses the
  ...
  documented as observability, not a delivery guarantee, and the exception message (now
  interpolated explicitly) still names the underlying error; an operator debugging a real
  Supabase outage has other, louder signals (the actual tool calls failing) to go on.
