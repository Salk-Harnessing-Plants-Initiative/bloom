...
populated — unlike `get_run`/`list_runs`, which always return `output_links == {}`. This is a
...
persisted (`output_sha256`, `output_keys`) or resolved fresh at call time (`url`,
`size_bytes`).
...
short-circuit, and SHALL produce a real (not fabricated) `size_bytes` for any run it itself
recorded via its own private, in-memory record of each output's byte size captured at commit
time (identical `hash_outputs` computation the real adapter also performs) — without making
any call to `StorageBackend`, since it never uploads real bytes for a live lookup to
meaningfully target.
...
- **THEN** it returns a `StoredRun` whose `output_links` carries a fresh, working `url` per
  output, each with the correct `sha256` and a live-resolved `size_bytes`
...
- **THEN** it resolves the same run `get_run(experiment, tool_class, run_ref)` would, with
  freshly signed `output_links` attached
...
#### Scenario: A legacy run with no recorded keys yields no links, not an error
...
- **THEN** it returns the resolved `StoredRun` with `output_links == {}`, without raising
...
- **THEN** every `size_bytes` comes from that run's own recorded byte size (captured
  ...
  with no call to `StorageBackend` of any kind
