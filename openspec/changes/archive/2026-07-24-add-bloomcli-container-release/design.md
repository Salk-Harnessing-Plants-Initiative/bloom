...
| `.trivyignore`'s existing base-image CVE suppressions now also apply to `bloomcli` (shares `langchain-agent`/`bloommcp`'s `python:3.11-slim` digest) without having been re-attributed (found in PR review) | `scripts/lint_cve_isolation.sh` enforces that any `.trivyignore` change ships in its **own** PR (only alongside Dockerfiles/lockfiles) — CI caught this when the fix first landed bundled with everything else in this PR. The `affects:` comment update (adding `bloomcli` to the ~11 shared-base-image entries; existing reasoning like "does not execute Perl/GnuTLS/SSH at runtime" transfers even more cleanly to a CLI with less runtime surface) is deferred to a separate, standalone PR per that policy, not skipped. |
