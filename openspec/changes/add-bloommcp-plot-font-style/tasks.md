...
- [ ] 1.1 `test_apply_font_style_noop_when_both_none` — call `apply_font_style` with a bare
...
- [ ] 1.4 `test_apply_font_style_family_only_leaves_size_unchanged` — call with only
  ...
  matplotlib default
- [ ] 1.4b `test_apply_font_style_size_only_leaves_family_unchanged` — symmetric case: call
...
- [ ] 1.5b `test_apply_font_style_applies_to_every_axes_on_multi_axes_figure` — build a
...
- [ ] 1.7 Regression check (not a new test — verify as part of running the existing suite in
...
- [ ] 1.11 Regression check (not a new test — verify as part of running the existing suite in
...
- [ ] 3.3 Update `generate_figures`'s docstring to mention the font-style post-processing step
...
- [ ] 4.4 Update `pca_analysis.py`'s module docstring "Optional plots (#426)" paragraph to
...
- [ ] 4.5 Same for `umap_analysis.py`'s "Optional plots (#425, reusing #426's shared helper)"
  module-docstring paragraph

## 5. Validate

- [ ] 5.1 Run `openspec validate add-bloommcp-plot-font-style --strict` — no issues
- [ ] 5.2 Run the test suite exactly as CI does:
