...
- [x] 2.2 Implement the function in **the same migration file as Task 1** (`20260912110000_add_cyl_writeback_run_scan_status.sql` — same concern: per-scan write-back status), as `SECURITY DEFINER`, triple-revoke (`PUBLIC`, `anon`, `authenticated`) + single-grant (`bloom_workflows`), matching `fail_cyl_pipeline_batch`'s existing convention.
...
- [x] 10.2 Fix the real semantic conflict the merge surfaced: `repin-cyl-contract-a7` (bloom #685)
  ...
  `'0.1.0a7'` in both `20260912110000_add_cyl_writeback_run_scan_status.sql` and its rollback.
- [x] 10.3 Rename both migration/rollback pairs from `20260901000000`/`20260901010000` to
  `20260912110000`/`20260912111000` (and every reference to those filenames — the test file's
...
- [x] 10.5 A fourth `/review-pr` round, run after CI went fully green, found two documentation
  ...
  - Added `status_update_matched` (`true`/`false` when `p_argo_workflow_name` was supplied, `null`
    ...
    `supabase/migrations/20260912110000_add_cyl_writeback_run_scan_status.sql`. 5 new integration
...
- [x] 14.6 Fixed CI: retimestamped both migrations (`origin/staging` had moved past
  `20260912110000`/`20260912111000` again in the days since Section 13's rebase) and rebased onto
  current `staging`.
