...
- [ ] 0.1 Add `bloommcp/data/` and `/=0.1.0a4` to `.gitignore` (both are untracked and **not** currently
...
- [ ] 1.1 Confirm the pin already satisfies the delegates: `bloommcp/pyproject.toml` pins
...
- [ ] 1.2 Add a **committed generator** (`bloommcp/scripts/gen_clustering_golden.py`) that emits
...
- [ ] 2.1 Add `bloommcp/tests/tools/test_clustering_tool.py`. Wire ports via a fixture that calls
...
- [ ] 2.2 **Determinism oracle (write FIRST):** for each `method in {"kmeans","gmm"}`, invoke
      ...
      tolerance.**
- [ ] 2.3 **Characterization snapshot through the tool:** kmeans (`n_clusters=3, seed=42`) →
...
- [ ] 2.4 Assert **no silent sample loss**: `result.n_samples == 153` (the certified row count), so a
      regression that let the delegate `dropna()` rows fails here.
- [ ] 2.5 **Default selection uses the full certified set (IM-1):** omit `trait_columns` → assert
...
- [ ] 3.1 **tools/list presence:** a FastMCP `Client` over the server lists `clustering` with a
      schema-bearing input model; assert `run_clustering_workflow` is still listed.
- [ ] 3.2 **Polymorphic delegation pinning + seed-reaches-fit (BL-2):** spy
...
- [ ] 3.3 **Provenance records the resolved seed (the new-for-this-tier path):** after a successful call
      ...
      records `None`.)
- [ ] 3.4 **Schema round-trip:** valid request ↔ input schema and result ↔ output schema round-trip
      without loss, for both methods.
- [ ] 3.5 **Method-exclusive scalars (IM-2):** a `method="kmeans"` result has `inertia is not None` and
...
- [ ] 3.6 **Cluster-count override + auto-select:** kmeans `n_clusters=4` → `result.n_clusters==4` and
...
- [ ] 3.7 **Invalid input — out-of-range / wrong-method control (split):** one assertion each — kmeans
      `n_clusters < 2` → validation code; gmm `n_components < 1` → validation code; `covariance_type`
      supplied with `method="kmeans"` → `invalid_input`; `n_clusters` supplied with `method="gmm"` (or
      `n_components` with `method="kmeans"`) → `invalid_input` naming the mismatch. No run persisted.
- [ ] 3.8 **Invalid input — trait columns (split, via `_qc_shared` with `require_certified=True`):** (a)
...
- [ ] 3.9 **Degenerate fit → structured, not internal:** using the **real** delegate on a degenerate
...
- [ ] 3.10 **Non-finite guard:** a selected certified trait carrying `±inf` (which `dropna()` keeps) →
      `assumption_violated`, no run persisted.
- [ ] 3.11 **Require-clean consumption (property):** with only a **raw** version registered → a
...
- [ ] 3.12 **`qc_inspect` non-regression (guards BL-1):** assert `qc_inspect`'s trait-subset validation
...
- [ ] 4.1 Add `bloommcp/src/bloom_mcp/tools/clustering_tool.py`: `ClusteringParams` (`experiment`,
      `method: Literal["kmeans","gmm"]`, `trait_columns?`, `standardize=True`, `seed=42` with `ge=0`,
      kmeans `n_clusters?` with `ge=2` + `max_clusters=10`, gmm `n_components?` with `ge=1` +
      `max_components=5` + `covariance_type: Literal["full","tied","diag","spherical"]="full"`,
...
- [ ] 4.2 Tool fn `clustering(params, *, random_state: int, provenance: Provenance) -> ClusteringResult`
...
- [ ] 4.3 **Catch-and-remap for specific codes/remedies:** catch `CleanedVersionRequiredError` → `raise
      ...
      codes.
- [ ] 4.4 `register(mcp)` via `bloom_mcp.contract.register`; register in `bloommcp/src/bloom_mcp/server.py`
...
- [ ] 4.5 **Shared-validator promotion (BL-1, `require_certified` opt-in):** promote the certified-set /
...
- [ ] 5.1 Persist via `store().create_run(experiment=…, tool_class="clustering", provenance=provenance,
...
- [ ] 5.2 Return the cluster summary inline + `run_ref` / `version_dir` / `manifest_path` / `outputs`. Add
...
- [ ] 6.1 Extend `bloommcp/scripts/live_persistence_smoke.py` with a `clustering` leg: after the existing
...
- [ ] 6.2 Factor the leg's pure decision logic into importable helpers and unit-test them in
...
- [ ] 6.3 **Update the live-smoke enumeration docs so they don't go stale (the `remove_outliers` sibling
...
- [ ] 8.1 **Validate on Claude Desktop (capable model) and sanity-check a small model** (a #309
      ...
      gmm-shaped summary.
- [ ] 8.2 Update the `server.py` module docstring (done in 4.4). Do **not** edit
...
- [ ] 8.3 `wsl -d Ubuntu -- bash -lc 'cd ~/repos/bloom && npx --yes -p @fission-ai/openspec openspec
      validate add-bloommcp-clustering-tool --strict'` is clean.
- [ ] 8.4 Tests + lint green: `cd bloommcp && uv run --frozen --extra test pytest
      ...
      **and the full bloommcp suite**; `uv run black --check .`, `uv run ruff check .`, and `pre-commit run
      --all-files` (or `/lint`).
- [ ] 8.5 `/pre-merge` → `/pr-description` → PR to `staging` linking #309 (stage by explicit path;
