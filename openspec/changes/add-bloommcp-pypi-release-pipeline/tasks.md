...
- [ ] 1.1 Add `bloom_mcp.__version__` to `bloommcp/src/bloom_mcp/__init__.py` via
...
- [ ] 1.3 Add `bloommcp/tests/test_version.py`:
  ...
  - `bloom-mcp --version` (and `-V`) exits/returns without invoking env validation, verified
    ...
    `SUPABASE_*` env set.
- [ ] 1.4 Commit: `feat(#663): add bloom_mcp.__version__ and --version/-V flag`.

## 2. Changelog and release runbook

- [ ] 2.1 Create `bloommcp/CHANGELOG.md` (Keep a Changelog format + the same PEP 440
...
- [ ] 2.2 Create `bloommcp/RELEASE_PROCESS.md`, mirroring `bloomcli/RELEASE_PROCESS.md`'s
...
- [ ] 2.3 Commit: `docs(#663): add bloommcp CHANGELOG.md and RELEASE_PROCESS.md`.

## 3. Package metadata

- [ ] 3.1 Add `[project.urls]` (Homepage, Repository, Changelog) to
      `bloommcp/pyproject.toml`, matching `bloomcli`'s convention.
- [ ] 3.2 Commit: `chore(#663): add [project.urls] to bloommcp/pyproject.toml`.
- [ ] 3.3 Bump `bloommcp/pyproject.toml`'s `version` from `0.1.0` to `0.1.0a1`.
- [ ] 3.4 Run `cd bloommcp && uv lock` to regenerate `bloommcp/uv.lock` (it records the
...
- [ ] 3.5 Commit: `chore(#663): bump bloommcp version to 0.1.0a1`.

## 4. Version-bump workflow

- [ ] 4.1 Add `.github/workflows/version-bloommcp.yml`, mirroring `version-bloomcli.yml`:
...
- [ ] 4.2 Commit: `feat(#663): add version-bloommcp.yml`.

## 5. Release workflow

- [ ] 5.1 Add `.github/workflows/release-bloommcp.yml`, mirroring `release-bloomcli.yml`'s
...
- [ ] 5.2 Commit: `feat(#663): add release-bloommcp.yml with its own tag-prefix guard`
...
- [ ] 6.1 Add the symmetric job-level guard to `.github/workflows/release-bloomcli.yml`'s
...
- [ ] 6.2 Update `bloomcli/RELEASE_PROCESS.md` to document only the `bloomctl-vX.Y.Z` tag form
...
- [ ] 6.3 Extend `tests/unit/test_release_bloomcli_workflow_shape.py` with an assertion that
      `validate-release`'s job-level `if:` contains the `bloomctl-` `startsWith` guard
      alongside `github.event_name != 'release'`.
- [ ] 6.4 Commit: `fix(#663): scope release-bloomcli.yml to bloomctl- tags` (isolated,
...
- [ ] 7.1 Add `tests/unit/test_release_bloommcp_workflow_shape.py`, mirroring
...
- [ ] 7.2 Commit: `test(#663): add release-bloommcp.yml workflow-shape regression guard`.

## 8. Validation

- [ ] 8.1 `openspec validate add-bloommcp-pypi-release-pipeline --strict`.
- [ ] 8.2 `cd bloommcp && uv run --extra test pytest tests/test_version.py -v`.
- [ ] 8.3 `cd bloommcp && uv run --extra test pytest tests/ -m "not integration and not live_smoke" -v`
      (confirm the `__init__.py`/`server.py` edits don't regress `test_package_baseline.py` or
      other fresh-import-sensitive tests).
- [ ] 8.4 `uv run --extra test pytest tests/unit/test_release_bloomcli_workflow_shape.py tests/unit/test_release_bloommcp_workflow_shape.py -v`.
- [ ] 8.5 `python scripts/check-uv-locks.py` (confirms `bloommcp/uv.lock` is back in sync after
      task 3.4's regeneration).
- [ ] 8.6 `uvx ruff@0.9.9 check bloommcp/`.
