...
Today's cyl trait read surface (`get_scan_traits`) is per-trait: loading one experiment means one call
per distinct trait name. The bloom#483 cylinder fixture is 129 samples × 880 raw trait columns (123 × 649
post-QC) — 649–880 round trips to load a single experiment, not viable for a wide-pivot read. This is
Tier 1 of `bloommcp`'s data-access roadmap
([bloom#546](https://github.com/Salk-Harnessing-Plants-Initiative/bloom/issues/546)), a prerequisite for
Tier 2's rewrite of `SupabaseReader`'s raw tier. It adds the missing bulk fetch, reusing the existing
latest/source/run selection logic (`cyl_scan_traits_source`, shipped
`20260701000000_cyl_trait_read_source_aware.sql`) rather than re-deriving it.
...
- Affected specs: `cyl-trait-read` (existing capability — additive; the six requirements shipped by
...
- Affected code:
  ...
  - `tests/integration/test_cyl_experiment_traits.py` (new).
  - `_WIKI/BLOOMMCP/README.md`'s "Supabase data access" section and
    `bloommcp/docs/data-access-roadmap.md`'s Tier 1 row + "Questions for Benfica" Q2 (docs only).
