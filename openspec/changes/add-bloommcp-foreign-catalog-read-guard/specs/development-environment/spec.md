...
- **WHEN** `.env.dev.example` documents `BLOOM_STORAGE_BACKEND`,
  `BLOOM_STORAGE_LOCAL_ROOT`, `BLOOM_EXPERIMENT_LOCAL_ROOT`, and
...
`BLOOM_STORAGE_BACKEND`, `BLOOM_STORAGE_LOCAL_ROOT`,
`BLOOM_EXPERIMENT_LOCAL_ROOT`, and `BLOOM_STORAGE_ALLOW_FOREIGN_MANIFEST` via
`${VAR:-}` interpolation from the active env
...
- **THEN** they set `BLOOM_STORAGE_BACKEND=local` (and optionally
  `BLOOM_STORAGE_LOCAL_ROOT` / `BLOOM_EXPERIMENT_LOCAL_ROOT`, or
  `BLOOM_STORAGE_ALLOW_FOREIGN_MANIFEST=1` for a deliberate foreign-catalog
  inspection) in their own
