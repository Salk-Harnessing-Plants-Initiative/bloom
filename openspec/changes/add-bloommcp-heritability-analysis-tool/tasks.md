...
- [ ] 1.1 Confirm every delegate is importable on the pinned lock:
...
- [ ] 1.2 Record `bloommcp/tests/fixtures/turface_19_heritability_golden.json`: clean
      ...
      "fix" the golden.
- [ ] 1.3 Add the golden's entry to `bloommcp/tests/fixtures/README.md` next to
...
- [ ] 1.4 Verify no legacy runs exist under tool class `"heritability"` for any experiment this
...
- [ ] 1.5 Confirm from the **actual current source** (not memory) that `pca_analysis.py` and
...
- [ ] 2.1 Add `bloommcp/tests/tools/test_heritability_analysis_tool.py`; wire `_ports.configure(...)`
...
- [ ] 2.2 Write the **golden-through-the-tool** test FIRST: invoke `heritability_analysis` with no
...
- [ ] 2.3 Assert `var_genetic` / `var_residual` / `n_genotypes` / `n_observations` / `model_type`
      ...
      Confirm RED.
- [ ] 2.4 Assert `n_traits_requested == n_traits_reported == 19`, `n_failed == 0`,
      `failed_traits == []`, `nonfinite_traits == []`, and `mean_h2` matches the golden at the same
      tolerance as 2.2. Confirm RED.
- [ ] 2.5 Determinism: two identical calls in one process produce **bit-identical** `per_trait`
...
- [ ] 3.1 `tools/list` presence: a FastMCP `Client` lists `sleap_roots_heritability_analysis` with
...
- [ ] 3.2 Schema round-trip: valid input/output validate; missing `experiment` →
...
- [ ] 3.3 Provenance + links: a successful call stamps `Provenance` with the tool name, the selected
...
- [ ] 3.3b Source label + bounded payload: `result.source == "v1_cleaned"` and `!= "raw"`; and
      `dumped = result.model_dump(); assert not any(isinstance(v, (list, dict)) and len(str(v)) >
      5000 for v in dumped.values())`.
- [ ] 3.3c Persisted-JSON content (not just parseability): assert its top-level key set is exactly
...
- [ ] 3.4 Delegation pinning (spy on `calculate_heritability_estimates`): called **exactly once**
...
- [ ] 3.5 Un-cleaned input: a `FakeReader` raising `CleanedVersionRequiredError` on
...
- [ ] 3.5b `version` selector (#626 pattern): the field exists on the input model; omitting it
...
- [ ] 3.6 Trait-selection validation, reusing `_validate_trait_subset(require_certified=True)`
      ...
      call's `trait_cols`.
- [ ] 3.7 Genotype/replicate roles (D3): (a) `replicate_col=None` with a present `genotype_col` →
...
- [ ] 3.8 Run-level delegate error: monkeypatch the delegate to return `{"error": "Missing required
      columns: [...]"}` → `BloomMCPError`, no run persisted, the raw delegate string not echoed
      verbatim into the envelope.
- [ ] 3.8b Declared-error mapping + no-leak (mirrors `test_pca_analysis_tool.py`'s block, absent
...
- [ ] 3.9 Per-trait failure routing: monkeypatch the delegate to (a) return `{"error": ...}` for one
...
- [ ] 3.9b All traits fail: the delegate returns `{"error": ...}` for every requested trait → the
...
- [ ] 3.10 Non-finite routing (D5): monkeypatch the delegate to return, for separate traits, a
...
- [ ] 3.10b Missing-key routing (D5(a) — the zero-fill guard `bloommcp-packaging` requires):
...
- [ ] 3.11 50-trait cap: a `FakeReader` cleaned frame with 60 certified traits (synthetic,
...
- [ ] 3.12 Composition + versioning (Supabase-adapter harness over `_InMemoryObjectStore`,
...
- [ ] 3.13 Discovery: `list_existing_analyses` reports a committed `heritability` run and, on a
      `list_runs` failure for that class, names the public tool `heritability_analysis`
      (bloom#664/#669 pattern).
- [ ] 3.14 Cylinder-scale unit oracle (mirroring `test_descriptive_stats_tool.py`'s cylinder test —
...
- [ ] 3.15 `user_label` (slugged into the version dir; omitted-label case), and the one retained
...
- [ ] 4.1 Default path: `include_plots` omitted → no figure generated, `outputs` contains only
      `heritability.csv` + `heritability_result.json`, and `compare_trait_heritabilities` is never
      called.
- [ ] 4.1b Import-cleanliness on the default path (the spec requires it; `test_pca_analysis_tool.py`
      has the pattern): with `monkeypatch.setitem(sys.modules, "matplotlib", None)`, a default
      `include_plots=False` call still succeeds.
- [ ] 4.2 `plots` with `include_plots=False` is silently ignored (no error, no figure).
- [ ] 4.3 Key validation before commit (reusing `validate_plot_keys` verbatim): `plots=[]`, an
...
- [ ] 4.4 Plot/number consistency (the issue's headline oracle): with `include_plots=True` and both
...
- [ ] 4.4b Documented ordering divergence (D1): on a >50-trait frame, assert the inline `per_trait`
...
- [ ] 4.5 Threshold forwarding (D4): with `threshold=0.7`, assert `create_heritability_plot`
...
- [ ] 4.6 Laziness + delegation of the comparison table: `plots=["create_heritability_plot"]` alone
...
- [ ] 4.7 Variance-component guard (D7): a comparison frame where a **scored** trait has NaN
...
- [ ] 4.7b Empty comparison frame: every trait fails and `create_variance_decomposition_plot` is
...
- [ ] 4.8 Persistence + cleanup: with `include_plots=True`, each requested figure lands as a `.png`
...
- [ ] 4.9 Pagination end-to-end: a cleaned frame with >50 certified traits and
...
- [ ] 5.1 RED in `bloommcp/tests/tools/test_plots_helpers.py`: a resolved call returning a
      ...
      per-character pages.
- [ ] 5.2 Implement the expansion inside `generate_figures`'s per-key loop. **Record every page
...
- [ ] 5.3 Update `generate_figures`'s type hints and docstring to state the `Figure | list[Figure]`
...
- [ ] 5.4 Re-run the existing `pca_analysis` / `umap_analysis` / `clustering` plot tests unchanged —
      the backstop that this shared-module change is additive.

## 6. GREEN — implement `heritability_analysis`

- [ ] 6.1 Add `bloommcp/src/bloom_mcp/sections/sleap_roots/analysis/heritability_analysis.py`:
...
- [ ] 6.2 Implement `heritability_analysis(params, *, provenance)` under `@as_mcp_tool` (declares
...
- [ ] 6.3 Reject a falsy `frame.genotype_col` with `BloomMCPError(code="assumption_violated")`
      naming the resolved roles; pass `frame.replicate_col` through unchanged (`None` included).
- [ ] 6.4 Resolve `trait_cols`: `frame.trait_cols` when `params.trait_columns is None`, else
      `_validate_trait_subset(frame, params.trait_columns, params.experiment,
      require_certified=True)` then the caller's list.
- [ ] 6.5 Delegate **once**: `h2_raw = calculate_heritability_estimates(frame.df, trait_cols,
...
- [ ] 6.6 Scrub into a **copy** (D5): for each per-trait entry, if any of `heritability`,
...
- [ ] 6.7 Build rows from `result.per_trait`, preserving resolved `trait_cols` order; cap at
...
- [ ] 6.8 Optional plots — validate keys and generate figures **before** `create_run`, wrapped in
...
- [ ] 6.9 Persist: `snapshot_frame(frame.df)` → `create_run(experiment=…, tool_class=_TOOL_CLASS
...
- [ ] 6.10 Register: import in `sections/sleap_roots/__init__.py`, add to the single
...
- [ ] 6.11 Add `heritability_analysis.py` to `bloommcp/tests/test_persistence_import_guard.py`'s
...
- [ ] 7.1 Delete `analysis/plot_heritability_bar.py` and `analysis/plot_variance_decomposition.py`;
      remove both imports and both `register(...)` entries from `sections/sleap_roots/__init__.py`.
- [ ] 7.2 Delete `tests/smoke/test_plot_heritability_bar_smoke.py` and
      `tests/smoke/test_plot_variance_decomposition_smoke.py`.
- [ ] 7.3 Update `tests/tools/test_viz_tools.py`: drop the two retired tools' cases and their module
...
- [ ] 7.4 `_viz_shared.py`: update **both** module-docstring "5" references (the title line and the
...
- [ ] 7.5 Roster surfaces: `server.py`'s module docstring (also add the missing
...
- [ ] 7.5b `tests/test_devendor_invariants.py`: **move** `sleap_roots_plot_heritability_bar` /
...
- [ ] 7.5c Retirement guard (the spec's "The retired modules no longer exist" scenario, which had
...
- [ ] 7.6 Add `"heritability"` to `sections/core/list_existing_analyses.py`'s `TOOL_CLASSES` and
...
- [ ] 7.7 Update `tests/test_oracle.py`'s comment naming `viz_tools.plot_variance_decomposition` as
...
- [ ] 7.8 **Two greps, not one.** (a) `grep -rn "plot_heritability_bar\|plot_variance_decomposition"`
...
- [ ] 7.9 Resolve the count-claim surfaces (b) finds — known at drafting time:
      ...
      `test_viz_tools.py`.
- [ ] 7.10 Fix `_WIKI/BLOOMMCP/writing-a-new-tool.md`, which tells a contributor that adding a tool
...
- [ ] 7.11 `bloommcp/docs/roadmap.md`: the #462 reference is **not** a row in the tier table — it
...
- [ ] 7.12 Add the migration table to `bloommcp/docs/connecting-claude-code.md` (a new "Retired
      ...
      the replacement.
- [ ] 7.13 Amend the two **unarchived sibling proposals** whose ADDED requirements mandate the
...
- [ ] 7.14 **If PR #724 has landed by then**, also in this commit: drop the `heritability_bar` /
...
- [ ] 7.15 Run the suite; debug to GREEN **without** weakening the golden or any §4 consistency
      assertion.

## 8. Live smoke + local-validation docs

- [ ] 8.1 Add `bloommcp/tests/smoke/test_heritability_analysis_smoke.py` mirroring
      ...
      exercised live).
- [ ] 8.2 Add a `heritability_analysis` leg to `tests/smoke/live_persistence_smoke.py` through the
...
- [ ] 8.3 Add pure-logic unit tests for the new smoke helpers to
      `tests/scripts/test_live_persistence_smoke_logic.py`, matching the no-live-stack pattern.
- [ ] 8.4 Add a `heritability_analysis` leg section to `bloommcp/docs/local-validation.md`
...
- [ ] 8.5 Re-run `make bloommcp-smoke` (all legs green) and the smoke helper unit tests.

## 9. Refactor & verify

- [ ] 9.1 Refactor for clarity; confirm the server boots (`uv run python -c "import
...
- [ ] 9.2 `/pre-merge`, plus the gaps `/pre-merge` itself does not cover:
