...
- [ ] 1.1.1 `test_strong_pairs_report_overlap_and_ci` — a frame with two known strong pairs at
...
- [ ] 1.1.2 `test_strong_pairs_ordered_by_ascending_overlap` — with ≥ 3 strong pairs at distinct
...
- [ ] 1.1.4 `test_strong_pair_overlap_summaries_are_uncapped` — same over-cap frame: assert
      `strong_pair_overlap_min`/`_median`/`_max` match an independent computation over **all**
      strong pairs, not just the 50 reported, and that `_min == strong_correlation_pairs[0]
      .overlap_n`.
- [ ] 1.1.5 `test_fisher_ci_matches_closed_form` — oracle: for a known (r, n), `ci_low`/`ci_high`
      equal `tanh(arctanh(r) ± 1.96 / sqrt(n - 3))` to a tight tolerance.
- [ ] 1.1.6 `test_perfectly_collinear_pair_reports_null_ci` — two exactly collinear traits over
...
- [ ] 1.1.7 `test_ci_is_null_below_four_overlap` — exercises the `n <= 3` branch, unreachable
...
- [ ] 1.1.8 `test_result_and_manifest_are_strict_json` — round-trip both the result
...
- [ ] 1.1.9 `test_strong_pairs_empty_when_nothing_is_strong` — no |r| > 0.7 pair → empty list,
      both counts 0, and the three overlap summaries `None`.
- [ ] 1.1.10 `test_strong_pairs_ordering_is_deterministic` — two runs over the same frame give
...
- [ ] 1.2.1 `test_locally_constant_pair_is_named` — that frame: the pair is in
      `locally_constant_trait_pairs`, in neither `zero_variance_traits` nor
      `low_overlap_trait_pairs`, and in neither strong count.
- [ ] 1.2.2 `test_every_nan_cell_has_exactly_one_reason` — the taxonomy-totality property, on a
...
- [ ] 1.2.3 `test_low_overlap_and_zero_variance_pair_lands_in_one_bucket` — a pair that is
...
- [ ] 1.2.4 `test_locally_constant_capped_with_uncapped_count` — a "saturating" trait
...
- [ ] 1.2.5 `test_locally_constant_does_not_populate_heatmap_caveat` — only that bucket
...
- [ ] 1.2.6 `test_locally_constant_pairs_survive_a_manifest_json_round_trip` — mirroring the
...
- [ ] 1.3.1 `test_non_finite_trait_is_reported_as_zero_variance` — a trait that varies but
...
- [ ] 1.4.1 `test_min_corr_overlap_is_owned_not_aliased` — assert
...
- [ ] 1.4.2 Confirm the existing `test_low_overlap_boundary_at_min_periods` parametrization
      still passes unchanged — the boundary must not move.

### 1.5 Manifest

- [ ] 1.5.1 `test_new_disclosure_fields_stamped_into_manifest_params` — both new lists, the
...
- [ ] 2.2 Add `_MAX_STRONG_PAIRS_REPORTED = 50`, `_MAX_LOCALLY_CONSTANT_PAIRS_REPORTED = 50`,
...
- [ ] 2.3 Add to `PlotCorrelationMatrixResult`: `strong_correlation_pairs`,
...
- [ ] 2.4 Expand `zero_variance_traits`' description with the non-finite fourth case
      (`design.md` Decision 7). Description only — the guard is already correct.
- [ ] 2.5 Compute the strong-pair list next to the existing `high_pos`/`high_neg`, reusing
...
- [ ] 2.6 Implement the Fisher-z CI as a module-level helper returning `(None, None)` for
      `|r| >= 1` or `n <= 3`, and coerce both bounds through `_qc_shared._finite_or_none`.
- [ ] 2.7 Compute `locally_constant_trait_pairs` as the residual bucket: upper-triangle `NaN`
...
- [ ] 2.8 Stamp every new field into the persisted run's `params`, and pass them all into the
      returned `PlotCorrelationMatrixResult`.
- [ ] 2.9 Leave `heatmap_caveat`'s population condition **untouched** (`design.md` Decision 4).
...
- [ ] 3.2 Replace "Known, narrow taxonomy gap (not fixed, disclosed)" with the shipped
...
- [ ] 3.4 Keep the docstring's #768 paragraph coherent with its rewritten neighbours — it now
      sits next to fields that supply the numeric surface its remedy wants (`design.md`
      Decision 6).
- [ ] 3.5 Update the test module's docstring to cover the new fields.
- [ ] 3.6 Add the new fields to `bloommcp/tests/smoke/test_plot_correlation_matrix_smoke.py`'s
...
- [ ] 4.1 `cd bloommcp && uv run --extra test pytest tests/tools/test_plot_correlation_matrix_tool.py -q`
      — all green, including every pre-existing test unchanged.
- [ ] 4.2 `cd bloommcp && uv run --extra test pytest tests/tools/ -q` — no sibling regression, in
...
- [ ] 4.3 `cd bloommcp && uv run black --check src tests && uv run ruff check src tests`.
- [ ] 4.4 `openspec validate add-bloommcp-corr-pair-disclosure --strict`.
- [ ] 4.5 Re-run `benchmarks/corr_pair_disclosure_bench.py` and confirm `design.md`'s tables
      still match (it exits non-zero if the taxonomy fuzz finds an unexplained cell).
- [ ] 4.6 Re-read the diff against `design.md` — confirm no existing field's value changed and
