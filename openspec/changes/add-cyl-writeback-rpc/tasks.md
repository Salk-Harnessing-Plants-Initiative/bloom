...
- [ ] 1.1 Author `proposal.md`, `design.md`, and spec deltas (`cyl-trait-writeback`,
...
- [ ] 2.1 **Happy path**: a valid envelope writes one `cyl_trait_sources` row (non-null `name`,
...
- [ ] 2.2 **Source name**: `cyl_trait_sources.name` is non-null and deterministic — re-deriving from
  the same envelope yields the same label (from `pipeline_run_id` when present, else a stable
  key-derived label).
- [ ] 2.3 **Return value shape**: the returned `jsonb` has `{source_id, scan_id, trait_count,
  blob_count, was_noop}`; `was_noop` is false on first call, true on re-delivery.
- [ ] 2.4 **Idempotent re-delivery**: calling twice with the same envelope yields exactly one source
  row and no duplicate trait/blob rows.
- [ ] 2.5 **Immutable provenance**: re-delivery with the same `idempotency_key` but a different
  `metadata` payload does NOT overwrite the stored `cyl_trait_sources.metadata` (first-writer-wins).
- [ ] 2.6 **Re-delivery is a pure no-op (short-circuit)**: a second call with the same envelope
...
- [ ] 2.7 **Trait re-delivery does not raise**: re-delivering the same run does not raise on the
  `cyl_scan_traits` `(scan_id, source_id, trait_id)` uniqueness and creates no duplicate.
- [ ] 2.8 **contract_version mismatch is rejected**; matching version is accepted.
- [ ] 2.9 **Empty / absent idempotency_key is rejected**; **key != metadata->>'idempotency_key' is
  rejected**.
- [ ] 2.10 **Trait-name registry (auto-register)**: an unseen `TraitValue.name` creates a `cyl_traits`
...
- [ ] 2.11 **grain**: an envelope with a `TraitValue` `grain = "image"` is rejected; a `TraitValue`
  that **omits** `grain` is accepted as scan-grain and written.
- [ ] 2.12 **NaN/inf/null/overflow → NULL**: values of JSON null, numeric NaN/inf, the strings
...
- [ ] 2.13 **Scan resolution**: multi-image envelope whose `image_ids` all belong to one scan resolves
...
- [ ] 2.14 **Envelope self-consistency / structure**: a `traits[].scan_key` or `blobs[].scan_key`
  ...
  source row (counts = 0).
- [ ] 2.15 **All-or-nothing incl. registry**: an envelope carrying an **unseen** `TraitValue.name`
...
- [ ] 2.16 **Same key, different scan**: deliver envelope 1 (key `K`, scan `S1`), then envelope 2 (same
...
- [ ] 3.1 **Catalog hardening assertions**: `prosecdef = true`; `proconfig` pins `search_path`;
...
- [ ] 3.2 **EXECUTE grants (exact set)**: `PUBLIC` cannot execute; exactly `bloom_writer`,
  `service_role`, `bloom_admin` hold `EXECUTE` (via `has_function_privilege`).
- [ ] 3.3 **Sole-writer**: under `SET LOCAL ROLE bloom_writer` (and `bloom_user`/`authenticated`), a
...
- [ ] 3.4 `bloom_writer` retains `SELECT` on all three tables.

## 4. RED — update existing tests to the new truth

- [ ] 4.1 `tests/integration/test_cyl_scan_intermediates.py`: **invert** `test_writer_can_insert_and_update`
...
- [ ] 4.2 `tests/integration/test_contract_migration_match.py`: flip the three deferred rows
...
- [ ] 5.1 Add `supabase/migrations/<ts>_add_cyl_writeback_rpc.sql`:
  ...
  - **E**: `DROP POLICY IF EXISTS` the legacy `authenticated` INSERT policies
    ...
    policies intact.
- [ ] 5.2 Run `make migrate-local`; iterate the function until tasks 2–4 are GREEN.
- [ ] 5.3 **Per-migration `db push` idempotency**: assert applying the migration body a second time is a
...
- [ ] 6.1 Add `supabase/rollbacks/<ts>_add_cyl_writeback_rpc_rollback.sql`:
  ...
  names; ROLLBACK).
- [ ] 6.2 Update `_WIKI/SUPABASE/README.md`: amend the `bloom_writer` row's **grants cell** (no longer
...
- [ ] 6.3 Regenerate the tracked Supabase types (`make gen-types`, db-dev up + migration applied) so the
...
- [ ] 7.1 `openspec validate add-cyl-writeback-rpc --strict` passes.
- [ ] 7.2 Full integration suite green (`make test-integration`), including the new
  `tests/integration/test_cyl_writeback_rpc.py`, the migration-match check, and the updated change-C
  RLS tests.
- [ ] 7.3 Lint/format clean (`make lint` / repo formatters); `test_db_push_is_idempotent` green.
- [ ] 7.4 Confirm forward-only migration + rollback restores prior schema (function + policies) on a DB
