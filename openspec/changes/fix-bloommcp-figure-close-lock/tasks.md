...
- [x] 0.1 Confirm the 5 unlocked sites are still present on this branch's base, by pattern
...
- [x] 0.2 Confirm the issue's third bullet is stale (nothing to implement):
...
- [x] 0.3 Confirm `_close_figure` has no reader outside `remove_outliers.py`:
...
- [x] 0.4 **Mechanical nesting check — replaces reading the five sites.** Write a throwaway
...
- [x] 0.5 Confirm no *unwrapped* delegate call in either tool touches pyplot at the pinned
...
- [x] 1.1 In `bloommcp/tests/tools/test_qc_inspect_tool.py`, add
...
- [x] 1.3 In `bloommcp/src/bloom_mcp/sections/sleap_roots/analysis/qc_inspect.py`: add
...
- [x] 1.4 **Delete** `import matplotlib.pyplot as plt` from `qc_inspect.py`. After 1.3 it has
...
- [x] 1.5 Update `_render_report`'s docstring: it promises "All matplotlib figures the
...
- [x] 1.6 Run `pytest tests/tools/test_qc_inspect_tool.py -v`; confirm 1.1/1.2 pass and no
...
- [x] 2.1 Add `test_success_path_closes_figures_while_holding_the_figure_registry_lock`:
...
- [x] 2.2 Add `test_unknown_plot_key_close_holds_the_figure_registry_lock`: same spy, then
...
- [x] 2.3 Add `test_unselected_figure_discard_holds_the_figure_registry_lock`, pinning the
      ...
      either vacuous.
- [x] 2.4 In `bloommcp/src/bloom_mcp/sections/sleap_roots/analysis/remove_outliers.py`: add
...
- [x] 2.5 Run `pytest tests/tools/test_remove_outliers_tool.py -v` and confirm the four
...
- [x] 3.1 In `bloommcp/tests/tools/test_plots_helpers.py`, add
...
- [x] 3.2 Add `test_lock_comment_describes_no_outstanding_close_site`: read `_plots.py`'s
...
- [x] 3.3 In `bloommcp/src/bloom_mcp/tools/_plots.py`: add `import logging` +
...
- [x] 3.4 Rewrite the `FIGURE_REGISTRY_LOCK` comment's "Where that stands" list: delete the
...
- [x] 4.1 In `bloommcp/tests/tools/test_plots_helpers.py`, add
      ...
      or 2 lands.
- [x] 4.2 Folded into 4.1 — dropped. A separate `rg 'def _close_figure'` guard adds nothing:
...
- [x] 4.3 Add `test_no_plots_run_acquires_the_figure_registry_lock_not_at_all` and
...
- [x] 4.4 Add `test_a_multi_figure_cleanup_takes_exactly_one_acquisition` using the same
...
- [x] 4.5 Add `test_persistence_io_runs_without_the_registry_lock_held` (spec: "Persistence
...
- [x] 4.6 Add `test_cleanup_failure_does_not_mask_the_tools_own_error` to
...
- [x] 4.7 Survey only, no code: `rg -n 'acquire' bloommcp/tests` to confirm no test asserts a
...
- [x] 5.1 Run `openspec validate fix-bloommcp-figure-close-lock --strict` and resolve issues.
- [x] 5.2 Confirm no capability collision with the merged-pending-archive
...
- [x] 6.1 From `bloommcp/`, run CI's exact invocation:
...
- [x] 6.2 Run `pre-commit run --files <touched files>`. Expect `ruff --fix` to be a no-op on
      `qc_inspect.py` now that task 1.4 deleted the dead import rather than `# noqa`-ing it.
- [x] 6.3 Stage **by explicit pathspec only.** Six `test_data/*.csv` files show permanent
      phantom line-ending modifications in this repo and must never be committed; never use
      `git add -A` or `git commit -a` here.
- [x] 6.4 Before pushing, merge `origin/staging` **locally** (`git merge origin/staging`) —
