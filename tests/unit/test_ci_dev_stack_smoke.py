...
# Issue #472's `make bloommcp-plot-smoke` ordering assertion used to live here. That step
# called a bare-`mcp.tool()` plotting tool through the container to prove the real
# bind-mounted PLOTS_DIR write path worked. #466 moved three of those tools onto
# ResultStore persistence and #462 retired the last two, so nothing in bloom_mcp writes to
# PLOTS_DIR any more; the step, its script, and this assertion were removed together.
# Retiring the PLOTS_DIR plumbing itself (static mount, env validation, compose bind-mount)
