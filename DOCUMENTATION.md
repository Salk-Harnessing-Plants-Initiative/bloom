...
| **⚡ FastAPI service** | `http://fastapi.localhost` | FastAPI backend - Video generation, presigned URLs, and OpenAPI docs |
...
-- FastAPI → Kong: `http://kong:8000`
-- FastAPI → MinIO: `http://supabase-minio:9000`
...
All FastAPI endpoints are accessible at: `http://fastapi.localhost` (service runs in container `fastapi-app`)

Interactive documentation (Swagger UI) is available at: `http://fastapi.localhost/docs`
...
**Description:** Serves the FastAPI root. The FastAPI app also exposes autogenerated OpenAPI/Swagger UI at `/docs`.

**Response:** Returns an HTML documentation page with all available endpoints, their parameters, and examples.

**Example:**
```bash
curl http://fastapi.localhost/
```
...
**Example:**
```bash
curl http://fastapi.localhost/docs
```
...
**Description:** Test endpoint that serves the documentation page. Can be used to verify the FastAPI service is running and accessible.

**Response:** Returns the HTML documentation page.

**Example:**
```bash
curl http://fastapi.localhost/test
```
...
**Example:**
```bash
curl http://fastapi.localhost/supabaseconnection
```
...
**Example:**
```bash
curl http://fastapi.localhost/list_buckets
```
---
...
**Example:**
```bash
curl -X POST http://fastapi.localhost/generate_video \
...
**Example:**
```bash
curl -X POST http://fastapi.localhost/get_presigned_urls \
  -H "Content-Type: application/json" \
...
| **FastAPI service** | http://fastapi.localhost | - |
...
**Test CORS:**
```bash
curl -v -H "Origin: http://studio.localhost" http://fastapi.localhost/
```
...
│   ├── Dockerfile             # FastAPI container definition (builds image from ./fastapi)
│   └── static/
