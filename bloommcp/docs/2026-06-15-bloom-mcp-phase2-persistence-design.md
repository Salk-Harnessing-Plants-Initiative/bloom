...
- **ResultStore durability — manifest compare-and-swap + commit-failure cleanup** (tracked: #324) — **adapter-side guard + scoped DELETE grant, shipped.** `SupabaseResultStore.commit` now finalizes `version_id`/`version_dir`/every derived key together, before any upload, via a pre-upload manifest re-read (reallocating on collision, bounded) and a pre-write re-read (failing safely — cleanup + retry-by-caller — on a late collision); a failed commit's already-uploaded objects are best-effort deleted, backed by a new migration scoping `bloom_agent` a DELETE grant to `bloommcp_output/*` only. This closes both gaps for the realistic *sequential*-interleaving case (one process, one commit finishing before the next starts uploading). **Residual, still deferred:** genuinely *simultaneous* multi-instance writers whose upload windows overlap byte-for-byte still need either verified storage-api conditional-upload support or a Postgres-transactional version-ledger pointer — unchanged trigger: bloom-mcp scaling past a single instance. This change is also a **prerequisite for #325** (fake↔adapter failure-injection parity — "once #324 lands"): `FakeResultStore` intentionally still has no matching collision/failure-injection behavior.
