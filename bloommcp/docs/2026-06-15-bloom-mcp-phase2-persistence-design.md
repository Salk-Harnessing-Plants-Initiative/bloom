...
- **ResultStore durability — manifest compare-and-swap + commit-failure cleanup** (tracked: #324). Two robustness gaps inherited from `AnalysisWriter`, both *currently safe* under the one-container / single-process topology:
  - *No CAS/`flock`* (its own docstring documents the single-writer assumption) — concurrent `v<N>` writers can clobber. *Trigger:* bloom-mcp scales past a single instance → add conditional-write inside the adapter **via the Supabase Storage API's conditional-upload support** (storage-api in front of MinIO) — not the raw MinIO/S3 SDK, which bloommcp never touches.
  - *Orphaned objects on partial commit failure* — `commit()` uploads outputs then writes the manifest last (so `latest` never points at a half-written version — no corruption), but a failure mid-loop leaves the already-uploaded objects unreferenced; only local staging is cleaned. *Trigger:* same as above → best-effort delete-on-failure or a prefix reaper.
