...
**Status: APPROVED (Benfica, 2026-07-23) — Tiers 1 & 2 shipped** (PR
[#548](https://github.com/Salk-Harnessing-Plants-Initiative/bloom/pull/548) merged
2026-07-29, PR [#557](https://github.com/Salk-Harnessing-Plants-Initiative/bloom/pull/557)
merged 2026-07-31). **Tier 3's LLM-facing-text slice is in review** (PR
[#571](https://github.com/Salk-Harnessing-Plants-Initiative/bloom/pull/571), open, tracked
by [#552](https://github.com/Salk-Harnessing-Plants-Initiative/bloom/issues/552)); the
remaining Tier 3 cleanup (retire `BLOOM_TRAITS_DIR`) is tracked by
[#476](https://github.com/Salk-Harnessing-Plants-Initiative/bloom/issues/476) (open, not
yet implemented). Verified live 2026-07-31 — see the dated Reconciliation log entry.
...
2. ~~**Tier 1's RPC shape** (a bulk long-format RPC vs. a PostgREST embedded-join query for
   ...
   `20260701000000_cyl_trait_read_source_aware.sql` migration.~~ **Resolved 2026-07-29** —
   Benfica approved Option A (the bulk RPC) via her review of
   [PR #548](https://github.com/Salk-Harnessing-Plants-Initiative/bloom/pull/548).
...
4. **`get_experiment_summary_counts`'s signature type** ([bloom#625](https://github.com/Salk-Harnessing-Plants-Initiative/bloom/issues/625),
   the Tier 2 `list_experiments()` N+1 fix) — implemented as `bigint` for `experiment_id_`/
   `source_id_` (matching every sibling RPC on these tables) rather than the issue's own literal
   `int` sketch, same review gate as Tier 1's RPC-shape decision. Needs Benfica's confirmation
   before merge; see that change's
   `openspec/changes/fix-bloommcp-list-experiments-summary-rpc/design.md` Decision D1.
5. **2026-08-14 — `list_experiments()` timeout fix ([bloom#637](https://github.com/Salk-Harnessing-Plants-Initiative/bloom/issues/637)
   / [bloom#656](https://github.com/Salk-Harnessing-Plants-Initiative/bloom/issues/656)),
   `fix-cyl-scan-traits-latest-rollup`** — supersedes the `get_experiment_summary_counts`
   implementation #4 above landed; two points need her confirmation:
...
- **Adjacent, not-to-duplicate work (cross-referenced, not folded in):**
  ...
  - [bloom#476](https://github.com/Salk-Harnessing-Plants-Initiative/bloom/issues/476)
    (open — verified live 2026-07-31; see below) — retiring bloommcp's remaining
    `BLOOM_TRAITS_DIR` read bypasses (`qc_inspect_tool.py`, `supabase_reader.py`'s
    deprecated local fallback). Its OpenSpec proposal merged as
    [PR #530](https://github.com/Salk-Harnessing-Plants-Initiative/bloom/pull/530)
    (2026-07-27), but that PR is **docs/plan-only** — none of `tasks.md`'s implementation
    items are done, and nothing has actually changed in `supabase_reader.py`/
    `qc_inspect.py` yet. GitHub's merge-linked auto-close fired on PR #530's merge
    (2026-07-27T16:49), but the issue was manually **reopened the next day**
    (2026-07-28T16:43) since the proposal's own design recommends #476 stay open,
    re-scoped to depend on this roadmap's Tier 2. Now that Tier 2 has merged (PR #557,
    2026-07-31), #476's local-disk-fallback target is genuinely moot to implement against
    — it's the concrete remaining piece of Tier 3 (see Tiers table).
  - [bloom#474](https://github.com/Salk-Harnessing-Plants-Initiative/bloom/issues/474)
    (closed — verified live 2026-07-31) — asked whether `docker-compose.prod.yml`'s
    bind-mount-permission bug (fixed in dev via #472/#473) also hits prod/staging for
    `SLEAP_OUT_CSV`/`PLOTS_DIR`/`ANALYSIS_OUTPUT`; resolved and closed via PR #495
    (2026-07-22).
  - [bloom#477](https://github.com/Salk-Harnessing-Plants-Initiative/bloom/issues/477)
    (closed — verified live 2026-07-31) — asked to confirm `SLEAP_OUT_CSV`/`ANALYSIS_OUTPUT`
    are dead weight in staging/prod and stop mounting them (or rename for clarity);
    resolved and closed via PR #495 (2026-07-22), which renamed `SLEAP_OUT_CSV` to
    `TRAITS_DIR`. Its own findings, kept for context since Tier 3 still cites them:
    `ANALYSIS_OUTPUT` is already
...
### Update (2026-07-31) — close-the-loop pass: Tiers 1 & 2 both merged, Tier 3 partially underway

Verified every live-state claim in this doc against current `gh` state (not the doc's own
citations) before ticking anything:

- **Tier 1 shipped.** [PR #548](https://github.com/Salk-Harnessing-Plants-Initiative/bloom/pull/548)
  merged 2026-07-29. Its own tracking issue, [#546](https://github.com/Salk-Harnessing-Plants-Initiative/bloom/issues/546),
  was never closed (no closing keyword in the PR) — closed by hand as part of this pass.
- **Tier 2 shipped, and the follow-up flagged in the 2026-07-23 entry above was resolved
  during implementation.** [PR #557](https://github.com/Salk-Harnessing-Plants-Initiative/bloom/pull/557)
  merged 2026-07-31 and confirms the anticipated simplification: `SupabaseReader.load_experiment`
  is **DB-only** (parses `name` as `str(experiment_id)`; a non-numeric name is
  `ExperimentNotFoundError`, never a local-disk fallback) — the two-ID-shape dispatch this
  doc's design notes describe was considered and dropped during PR review (see PR #557's
  own description, design.md §D1). Its tracking issue, [#551](https://github.com/Salk-Harnessing-Plants-Initiative/bloom/issues/551),
  was likewise never auto-closed — closed by hand as part of this pass.
- **Tier 3 has no dedicated tracking issue and none is being filed yet** — per the
  just-in-time policy, the tier's actual scope is already covered by two existing issues
  reached independently: [#552](https://github.com/Salk-Harnessing-Plants-Initiative/bloom/issues/552)
  (LLM-facing "CSV filename" wording; PR [#571](https://github.com/Salk-Harnessing-Plants-Initiative/bloom/pull/571)
  open, not yet merged) and [#476](https://github.com/Salk-Harnessing-Plants-Initiative/bloom/issues/476)
  (retire `BLOOM_TRAITS_DIR`; still open, implementation not started despite its OpenSpec
  proposal — PR #530 — having merged 2026-07-27).
- **[Factual, real gap] #476's status was stale in two ways.** The doc's Live-state facts
  bullet still said "open, filed 2026-07-20" with no mention of PR #530. Checked the
  issue's timeline directly (`gh api .../issues/476/timeline`): PR #530's merge
  auto-closed #476 on 2026-07-27T16:49, and it was manually reopened the next day
  (2026-07-28T16:43) — consistent with the proposal's own recommendation that #476 stay
  open pending Tier 2. → **Reconciled:** bullet rewritten with the verified timeline and
  PR #530's docs-only scope stated explicitly.
- **[Factual, real gap] #474 and #477 were both already closed**, via
  [PR #495](https://github.com/Salk-Harnessing-Plants-Initiative/bloom/pull/495)
  (2026-07-22, renamed `SLEAP_OUT_CSV` → `TRAITS_DIR`), but this doc still listed both as
  "open" in the Live-state facts cross-reference list. → **Reconciled:** both bullets
  updated to closed, with the resolving PR cited; their prior findings (kept for context
  since Tier 3's row still cites them) left intact.
- Deliberately **not** re-litigated in this pass: #406 (per-user identity), #388 (file
  upload/download), and #374 (source-blind consumer repoint) are cross-referenced by this
  doc but out of its own tier scope — left for their own threads. #406's implementation
  merged as PR #563 (2026-07-30) but its tracking issue is a separate concern from this
  roadmap and wasn't touched here.
