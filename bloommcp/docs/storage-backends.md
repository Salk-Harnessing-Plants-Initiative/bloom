...
This can't be _prevented_ from purely local information — the `local` backend never queries
`supabase`'s catalog directly, and has no way to check whether `supabase` already has
history for an experiment (and vice versa) without doing so, which would
