...
To add a section: create ``bloom_mcp/sections/<name>.py`` exposing a
``section`` FastMCP instance, then add it to ``SECTIONS`` below. That is the
only server-side wiring — no ``server.py`` edit per tool.
"""
