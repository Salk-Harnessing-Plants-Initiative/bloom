...
def test_golden_stats_through_the_tool(injected_ports):
    """2.2 — reproduce the independently-computed golden through the MCP tool."""
    result = _run()

    golden = _GOLDEN["stats_per_trait"]["Shoot_Biomass_mg"]
    s = _stat(result, "Shoot_Biomass_mg")
    assert s.n == golden["count"] == 158
    assert s.mean == pytest.approx(golden["mean"], abs=1e-9)
    assert s.std == pytest.approx(golden["std"], abs=1e-9)
    assert s.median == pytest.approx(golden["median"], abs=1e-9)
    assert s.q25 == pytest.approx(golden["q25"], abs=1e-9)
    assert s.q75 == pytest.approx(golden["q75"], abs=1e-9)
    assert s.min == pytest.approx(golden["min"], abs=1e-9)
    assert s.max == pytest.approx(golden["max"], abs=1e-9)
    assert s.cv == pytest.approx(golden["cv"], abs=1e-9)
    assert s.skewness == pytest.approx(golden["skewness"], abs=1e-9)
    assert s.kurtosis == pytest.approx(golden["kurtosis"], abs=1e-9)

    # Root_Shoot_Ratio: deliberately non-normal — must not be clipped/flagged.
...
def test_nonfinite_statistics_are_coerced_to_none_and_named(injected_ports):
    ...
    assert set(result.nonfinite_stat_traits) == {"constant_trait", "zero_mean_trait"}

    # Persisted CSV agrees: empty cell, not a literal inf/nan token.
    stored = store.get_run("nonfinite.csv", "stats", "latest")
    csv_key = stored.output_keys["stats.csv"]
    csv_text = (
        Path(csv_key).read_text(encoding="utf-8") if Path(csv_key).exists() else None
    )
...
# ── 3.9b finiteness re-verification guard ────────────────────────────────────


def test_residual_nan_in_certified_trait_is_assumption_violated(monkeypatch):
    """A certified trait carrying a residual NaN (a reader/qc_clean-invariant
    violation) is rejected before delegating -- the delegate must never silently
    under-count n."""
    reader = FakeReader()
    store = FakeResultStore()
    bad_df = pd.DataFrame(
    ...
    try:
        with pytest.raises(BloomMCPError) as exc:
            descriptive_stats(DescriptiveStatsParams(experiment="nanleak.csv"))
        assert exc.value.code == "assumption_violated"
        assert store.list_runs("nanleak.csv", "stats") == []
        assert called["n"] == 0
    finally:
        _ports.configure(reader=SupabaseReader(), store=SupabaseResultStore())
