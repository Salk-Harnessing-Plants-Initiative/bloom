"""Old v2/v3/v4 manifests still read under v5 code — the additive bumps hold.

Maps the spec "Additive Manifest Schema v4" back-compat scenarios (plus the
v5 `storage_backend` sentinel, #395): a recorded v2 manifest (string-valued
``outputs``, no v3/v4/v5 fields), a recorded v3 manifest (no
``source_id``/``source_name``), and a recorded v4 manifest (no
``storage_backend``) all validate under current code with ``extra="forbid"``
still on, with their absent fields defaulting to unset.
"""
...
def test_recorded_v2_manifest_reads_under_v3():
    """The committed v2 fixture validates and its v3/v4/v5 fields are unset."""
    raw = json.loads(_FIXTURE.read_text())

    validate_schema(raw)  # version 2 <= known (5): accepted
    ...
    # v5 field absent in v2 defaults to unset.
...
def test_recorded_v3_manifest_reads_under_v4():
    ...
    validate_schema(raw)  # version 3 <= known (5): accepted
    manifest = Manifest.model_validate(raw)
    ...
    # v4/v5 fields absent in v3 default to unset.
    assert entry.source_id is None
    assert entry.source_name is None
    assert manifest.storage_backend is None


def test_recorded_v4_manifest_reads_under_v5():
    """A v4-shaped manifest (source_id/source_name, no storage_backend) still loads."""
    raw = {
        ...
        "versions": [
            {
                ...
                "outputs": {"loadings": "_loadings.csv"},
                "source_id": 7,
                "source_name": "reprocess-2026-07",
            }
        ],
        "latest": "v1",
    }

    validate_schema(raw)  # version 4 <= known (5): accepted
    manifest = Manifest.model_validate(raw)
    ...
    # v5 field absent in v4 defaults to unset.
    assert manifest.storage_backend is None
...
def test_newer_schema_version_is_rejected():
    ...
    with pytest.raises(ManifestSchemaError):
        validate_schema({"manifest_schema_version": 6})
