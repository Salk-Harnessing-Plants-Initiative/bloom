...
one-time migration-time population -- a real risk given the refresh workflow
is on-demand only (design.md D8), not automatic, for either environment. This
locks in that an elapsed time past `_STALE_AFTER` gets an explicit staleness
flag instead of reading like ordinary bounded lag.
"""
...
def test_just_under_the_stale_threshold_shows_plain_timestamp():
    ...
    assert "refresh on demand only" not in note
...
def test_past_the_stale_threshold_flags_it():
    ...
    assert "trait counts refresh on demand only" in note
