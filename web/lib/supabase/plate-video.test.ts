...
const SIGNED = {
  data: { signedUrl: 'http://kong:8000/storage/v1/object/sign/graviscan-videos/v.mp4?token=t' },
}
const PUBLIC_URL = 'https://bloom.example.org/storage/v1/object/sign/graviscan-videos/v.mp4?token=t'
...
describe('getStoredPlateVideoUrl', () => {
  ...
  it('gives back the URL when a video is stored', async () => {
    ...
    await expect(getStoredPlateVideoUrl(7, 'P1', 2)).resolves.toBe('https://cdn.example/v.mp4')
  })
