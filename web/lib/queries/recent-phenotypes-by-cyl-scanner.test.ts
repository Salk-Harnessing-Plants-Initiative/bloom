/**
 * Unit tests for getRecentExperimentsByCylScanner.
 *
...
import {
  getRecentExperimentsByCylScanner,
  type RecentExperimentRow,
} from "@/lib/queries/recent-experiments-by-cyl-scanner";
 ...
 * fields per test.
 */
function makeRow(overrides: Partial<RecentExperimentRow> = {}): RecentExperimentRow {
  return {
    scanner_id: 1,
    scanner_name: "Cyl-01",
    experiment_id: 100,
...
describe("getRecentExperimentsByCylScanner", () => {
  it("returns an empty array when the view returns no rows", async () => {
    const supabase = makeMockSupabase([]);
    const result = await getRecentExperimentsByCylScanner(supabase);
  ...
  it("groups a single scanner's rows into one section", async () => {
    ...
    const result = await getRecentExperimentsByCylScanner(makeMockSupabase(rows));
  ...
  it("throws with a descriptive message when supabase returns an error", async () => {
    ...
    await expect(getRecentExperimentsByCylScanner(supabase)).rejects.toThrow(
      /Failed to fetch recent experiments by cyl scanner.*relation does not exist/,
    );
  });

  it("queries the recent_experiments_by_cyl_scanner view", async () => {
    const supabase = makeMockSupabase([]);
    await getRecentExperimentsByCylScanner(supabase);
