...
export async function POST() {
  ...
  const supabase = createServiceRoleSupabaseClient(supabase_url, service_role_key) as any
  ...
  const { data: response, error } = await supabase.from('oauth_flow_state').insert([
    {
      user_id: user.id,
      provider,
      state,
