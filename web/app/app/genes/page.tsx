...
function AlertDialog({ openDialouge, sample_name, handleSubmitForm }: { openDialouge: boolean, sample_name: String, handleSubmitForm: () => void }) {
...
function FormContent({ ortho_group, nick_name, closeModalBox, selected_gene }: FormContentProps) {
  ...
  const handleFormSubmit = async () => {
    ...
    try {
      for (const gene of ortho_group) {
        const updatePayload = {
          standard_name: String(`${gene.assemblies?.prefix ?? "undefined"}-${nick_name ?? "undefined"}-${gene.ortho_group_row_number ?? null}`),
  ...
  return (
    ...
    >
      ...
      {message && <Alert severity="error" action={
        <CloseIcon
      ...
      <Box sx={{ display: "flex", flexDirection: "row", gap: 2, width: "100%" }}>
        ...
        <Box sx={{ flex: 1, border: "1px solid gray", p: 2, height: "200px", overflow: "scroll", borderRadius: "5px" }}>
          <Typography variant="h6" align="center">
            Gene Names
          </Typography>

          {ortho_group && ortho_group.map((gene) => {
            return (
              <Box key={gene.gene_id} sx={{ display: "flex", flexDirection: "row", gap: 2, width: "100%" }}>
      ...
      <AlertDialog
        ...
        sample_name={
          ortho_group?.[0]
            ? `${ortho_group[0]?.assemblies?.prefix || "SamplePrefix"}-${nick_name || "SampleNick"}-${ortho_group[0]?.ortho_group_row_number || "0"}`
...
function LookupModal({ modal_state, closeModal }: { modal_state: boolean, closeModal: () => void }) {
  const style = {
    position: 'absolute',
    top: '50%',
    left: '50%',
  ...
  const debouncedInputChangeLookup = _.debounce(
    ...
    <>
      ...
      >
        <Box sx={style}>
          ...
          <Box sx={{ display: 'flex', justifyContent: 'center', mb: 2 }}>
            <Typography variant="subtitle1" sx={{ color: 'text.secondary', mt: 1, textAlign: 'center' }}>
            ...
            <Autocomplete
              ...
              getOptionLabel={(option) =>
                typeof option === "string"
                  ? option
                  : `${option.gene_id} (${option.standard_name})`
              }
              ...
              renderInput={(params) => <TextField {...params} label="Search by gene nickname (standard name) or gene ID..." />}
            />
          </Box>
...
function ModalBox({ modal_box, openModal, closeModal }: { modal_box: boolean, openModal: () => void, closeModal: () => void }) {
  ...
  const debouncedInputChange = _.debounce(
    (_event: React.SyntheticEvent, value: string) => {
        ...
        >
          <Box sx={style}>
            <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', width: '100%' }}>
              <Typography id="modal-modal-title" variant="h6" component="h2" style={{ flex: 1, textAlign: 'center' }}>
            ...
            />
            {form && <FormContent ortho_group={ortho_group} nick_name={nick_name} closeModalBox={handleClose} selected_gene={selected_gene} />}
          </Box>
        </Modal>
      </div>
    </>
    ...
    <>
      ...
      >
        <List sx={{ pt: 1 }}>
          ...
          {/* Button 1 */}
          <Tooltip title="Add gene nickname (standard names) across gene families" placement="left" arrow>
            <ListItem
              component="button"
              sx={{
      ...
      <div>
        ...
        {/* Status counts */}
        <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'space-between', marginBottom: '20px', width: '100%', gap: '20px' }}>
          <TextField
            label="Search Genes"
            variant="outlined"
            value={searchQuery}
        ...
        <AddGeneCandidateModal modal_state={candidate_modal} closeModal={handleGeneCandidateModalClose} setNewCandidateAdded={setNewCandidateAdded}/>
        <LookupModal modal_state={lookup_modal} closeModal={handleGeneLookupModalClose} />
        <CurrentStatus open={currentStatus} setOpen={setCurrentStatus} selectedGene={selectedGene} />
        <div className="mb-6">
          {geneCandidates ? (
            <div className="flex flex-row">
              {Object.entries(getStatusCounts(geneCandidates)).map(
                ([status, count]) => (
                  <div
                    key={status}
                    className={
                  ...
                  >
                    ...
                    <div className="text-xs">
                      <Status status={status as Status} handleCurrentStatus={handleCurrentStatus} geneCandidate={"UNKNOWN"} setSelectedGene={setSelectedGene}/>
                    </div>
                  </div>
                )
              )}
        ...
        <table className="rounded-md">
          <thead>
            <tr>
              ...
              <th className="text-xs text-left px-2 pb-4 align-bottom">Current <br />  Status</th>
              <th className="text-xs text-left px-2 pb-4 align-bottom">
                Category
              </th>
              <th className="text-xs text-left px-2 pb-4 align-bottom">
          ...
          <tbody>
            {geneCandidates
              ...
              ?.filter(
                (row) => selectedStatus === null || selectedStatus === row.status
              )
              .map((row) => (
                <tr key={row.gene} className="odd:bg-stone-200">
                  <td className="text-left p-2">
                    {row.genes?.assemblies?.hpi_reference_id &&
                      row.genes?.short_id ? (
                      <a
                  ...
                  <td className="text-left p-2">
                    <Status status={row.status as Status} handleCurrentStatus={handleCurrentStatus} geneCandidate={row.gene} setSelectedGene={setSelectedGene}/>
                  </td>
                  ...
                  <td className="text-left p-2">
                    <Progress candidate={row} candidates_list={geneCandidates} currentGeneCandidate={currentGeneCandidate} isOpen={openCandidate === row.gene || openCandidate ==="generic" }  handleOpen={() => handleExperimentLogsOpen(row.gene)} handleExperimentLogsClose={handleExperimentLogsClose} handleCandidateChange={handleCandidateSelectionChange}/>
                  </td>
                  <td className="text-left p-2">
