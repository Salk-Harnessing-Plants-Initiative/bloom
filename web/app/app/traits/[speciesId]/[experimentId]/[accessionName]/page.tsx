...
export default async function Accession({
  ...
  params: { experimentId: number; accessionName: string }
}) {
  ...
  return (
    <div className="">
      ...
      <div className="table-auto select-none">
        {plants?.map(
          (
            plant: { id: Key | null | undefined; qr_code: any; cyl_scans: any[] },
            index: number
          ) => (
            <div className="table-row" key={plant.id}>
              ...
              <div className="table-cell text-lg align-middle pb-4">
                <div className="text-sm mt-2 text-neutral-400 flex flex-row">
                  {plant.cyl_scans
                    .sort((a, b) => (a.plant_age_days ?? 0) - (b.plant_age_days ?? 0))
                    .map((scan) => (
                      <div key={scan.id} className="mr-4">
                        {scan.cyl_images.map(
                          (image: { id: Key | null | undefined; object_path: string | null }) => (
                            <div key={image.id} className="text-center">
                              <div className="pb-1">Day {scan.plant_age_days}</div>
                              <Link
                                href={`/app/phenotypes/${species?.id}/${experiment?.id}/${params.accessionName}/${image.id}`}
                              >
