...
export default async function Translation() {
  ...
  return (
    <div>
      ...
      <div className="mb-6 select-none">Information about different translation projects.</div>
      <div>
        <table className="mt-6">
          <thead>
            <tr>
              <th className="text-sm text-neutral-600 text-left">Project name</th>
              <th className="text-sm text-neutral-600 text-left pl-4">Spreadsheet</th>
            </tr>
          </thead>
          <tbody>
            {projects?.length === 0 ? (
        ...
        {projects?.length === 0 && (
          <div className="text-sm mt-6 text-neutral-600">
            You don't have access to any projects. Email Dan (dbutler@salk.edu) to request access.
          </div>
        )}
      </div>
    </div>
