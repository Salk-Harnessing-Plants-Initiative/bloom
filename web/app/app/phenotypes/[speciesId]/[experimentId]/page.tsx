  ...
  return (
    <div className="">
      <div className="text-xl mb-8 select-none">
        <span className="text-stone-400">
          ...
          <span className="hover:underline capitalize">
            <Link href={`/app/phenotypes/${experiment?.species?.id}`}>{speciesName}</Link>
          </span>
          &nbsp;▸&nbsp;
        </span>
      ...
      <div className="text-lg align-middle">
        ...
        <div
          className={
            'table-auto pl-4 text-sm ' + (experiment?.cyl_waves.length === 1 ? 'mt-8' : 'mt-2')
          }
        >
                    ...
                    {getAccessions(safeWave).map(({ name, count, id }) => (
                      <div className="table-row" key={'accession-' + id}>
                        <div className="table-cell">
                          ...
                          >
                            <span className="text-lime-700 hover:underline">{name}</span>
                          </Link>
                        </div>
                        <div className="table-cell pl-8">
                          <span className="text-neutral-400">&nbsp;{count} replicates</span>
                        </div>
                      </div>
                    ))}
                  </div>
