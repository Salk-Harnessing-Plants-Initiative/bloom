...
const geneList = ["Gene1", "Gene2", "Gene3", "Gene4", "Gene5", "Gene6", "Gene7", "Gene8", "Gene9", "Gene10", "Gene11", "Gene12", "Gene13", "Gene14", "Gene15", "Gene16", "Gene17", "Gene18", "Gene19", "Gene20", "Gene21", "Gene22", "Gene23", "Gene24", "Gene25", "Gene26", "Gene27", "Gene28", "Gene29", "Gene30", "Gene31", "Gene32", "Gene33", "Gene34", "Gene35", "Gene36", "Gene37", "Gene38", "Gene39", "Gene40"];
...
export default function CorrelationAnalysis({ file_id }: { file_id: number }) {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const { data: gene_list_res, error: gene_list_error } = await supabase.from("scrna_genes").select("id, gene_name").eq("dataset_id", file_id);
    ...
    const fetchCountsData = async (gene_id: number, gene_name: string) => {
        try {
            ...
            if (geneData?.[0]?.counts_object_path) {
                const { data: storageData } = await supabase.storage.from("scrna").download(geneData[0].counts_object_path);
        ...
        >
            <div style={{ padding: "10px", minWidth: "180px" }}>
                ...
                <ul style={{ listStyle: "none", padding: 0, margin: 0 }}>
                    <li><Button>🌱 Cell Type 1</Button></li>
                    <li><Button>🌿 Cell Type 2</Button></li>
                    <li><Button>🌾 Cell Type 3</Button></li>
                    <li><Button>🌰 Cell Type 4</Button></li>
                    <li><Button>🌱 Cell Type 5</Button></li>
                    <li><Button>🍃 Cell Type 6</Button></li>
                    <li><Button>🪵 Cell Type 7</Button></li>
                </ul>
            </div>
        </Popover>
    )
    ...
    const drawScatterPlot = (genes: string[]) => {
        ...
        for (let i = 0; i < numGenes; i++) {
            for (let j = 0; j < numGenes; j++) {
                ...
                const geneXValues = Object.values(geneData[geneX] as { [key: number]: number }).map(val => Number(val !== undefined ? val : 0));
                const geneYValues = Object.values(geneData[geneY] as { [key: number]: number }).map(val => Number(val !== undefined ? val : 0));
                
                let x = d3.scaleLinear().domain([0, d3.max(geneXValues) as number]).range([0, size - 20]);
                let y = d3.scaleLinear().domain([0, d3.max(geneYValues) as number]).range([size, 0]);

                let cell = svg.append("g")
                    .attr("transform", `translate(${(position(geneX.toString()) ?? 0) + 50 + (40 * i)}, ${(position(geneY.toString()) ?? 0)})`);
                ...
                cell.selectAll("circle")
                    ...
                    .append("circle")
                    .attr("cx", barcode => {
                        const value = (geneData[geneX] as { [key: string]: number })[barcode] !== undefined ? (geneData[geneX] as { [key: string]: number })[barcode] : 0;
                        geneXCounts.push(value)
                        return x(value);
                    })
                    .attr("cy", barcode => {
                        const value = (geneData[geneY] as { [key: string]: number })[barcode] !== undefined ? (geneData[geneY] as { [key: string]: number })[barcode] : 0;
                    ...
                    .on("mouseover", function (event, barcode) {
                        const valueX = (geneData[geneX] as { [key: string]: number })[barcode] !== undefined ? (geneData[geneX] as { [key: string]: number })[barcode] : 0;
                        const valueY = (geneData[geneY] as { [key: string]: number })[barcode] !== undefined ? (geneData[geneY] as { [key: string]: number })[barcode] : 0;
        ...
        <>
            <Box display="flex" height="100vh">
                ...
                <Box display="flex" flexDirection={"column"} sx={{ width: "15%" }}>
                    <Typography variant="h6" sx={{ textAlign: "center" }}>Select Genes</Typography>
                    <Box sx={{ width: "100%", height: "100vh", overflow: "auto", mt: 2, border: "1px solid #ddd", borderRadius: 2, p: 1 }}>
                        <FormGroup >
                            {geneNames && geneNames.map((gene) => (
                                <FormControlLabel
                                    key={gene.id}
                                    control={
                                        <Checkbox
                ...
                <Box width="85%" p={0} sx={{ display: "flex", alignItems: "flex-start" }}>
                    <div id="tooltip" style={{ position: 'absolute', visibility: 'hidden', backgroundColor: '#c0c2c0', borderRadius: '8px', boxShadow: '2px 2px 5px rgba(0, 0, 0, 0.3)' }}></div>
                    <svg style={{
                        width: '100vh',
                        height: '100vh',
                    }} ref={chartRef}></svg>
