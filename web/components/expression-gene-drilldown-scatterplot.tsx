...
export default function GeneDrillUMAP({
  scatterPlot,
  geneName,
}: {
  ...
  useEffect(() => {
    if (scatterPlot) {
      ...
      const legendHeight =
        (legendContainer ? legendContainer.clientHeight : 0) - margin.top - margin.bottom
      const expressionExtent = d3.extent(scatterPlot, (d: ScatterPlot) => d.expression) as [
        number,
        number,
      ]
      const colorScale = d3
      ...
      const yScale = d3
        .scaleLinear()
        .domain(
          d3.extent(scatterPlot, (d: ScatterPlot) => {
            return d.y
          }) as [number, number]
        )
        .range([height - 20, 20])
