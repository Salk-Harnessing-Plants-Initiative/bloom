...
export default function ExpressionGeneLevel({ file_id }: { file_id: number }) {
    ...
    const [selected_gene, setSelectedGene] = useState<{ gene_name: string; gene_number: number }>({ gene_name: '', gene_number: 0 });
        ...
        const fetchData = async () => {
            try {
                ...
                const { data: geneData, error: geneError } = await supabase
                    ...
                    .limit(6) as unknown as { data: Array<{ counts_object_path?: string; gene_id: number }>, error: any };
        ...
        const fetchData = async () => {
            ...
            if (gene_list_res && gene_list_res.length > 0) {
                ...
                const { data: geneData, error: geneError } = await supabase
                    ...
                    .eq("dataset_id", file_id) as unknown as { data: Array<{ counts_object_path?: string }>, error: any };
        ...
        <div>
            {file_id && initial_load && (
                <Autocomplete
                    ...
                    renderInput={(params) => (
                        <TextField {...params} label="SELECT GENE" placeholder="Add genes for a comparative view." />
                    )}
