...
export default function ExpressionGeneLevelBoxPlot({
  input_array,
  gene_name,
  gene_id,
  file_id,
  ...
  gene_id: number
  file_id: number
  setScatterPlotData: (data: any) => void
  handleshowplot: (value: boolean) => void
    ...
    if (plot_data) {
      const stats = Object.entries(plot_data).map(([key, value]) => {
        ...
        const cells = value.map((item) => ({
          ...
          barcode: String(item.barcode ?? 'NA'),
        }))
      ...
      svg
        ...
        .each(function (d) {
          ...
          d3.select(this)
            ...
            .style('fill', '#69b3a2')
            .on('mouseover', function () {
              d3.select(this).style('cursor', 'pointer').style('fill', 'blue')
            })
            .on('mouseout', function () {
              d3.select(this).style('cursor', 'default').style('fill', 'blue')
            })
            .on('click', () => {
              setDrillDownGene(d!.key)
            })
        ...
        >
          <label
            style={{
              ...
              alignItems: 'center',
              marginBottom: '10px',
              border: '2px solidrgb(251, 254, 255)',
              borderRadius: '5px',
            }}
          >
            <div
