...
export default function ExpressionMetadata({ file_id }: { file_id: number }) {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            ...
            const { data: metadata, error: metadataError }: { data: Metadata | null, error: any } = await supabase
                .from("scrna_datasets")
        ...
        <>
            <Box sx={{ display: 'flex', flexDirection: 'row', gap: 2, p: 2 }}>
                <Box sx={{
                    ...
                    <Typography variant="subtitle1" color="text.secondary">Assembly: {assembly}</Typography>
                    <Typography variant="subtitle1" color="text.secondary">Annotation: {annotation}</Typography>
                    <Typography variant="subtitle1" color="text.secondary">Strain: {strain}</Typography>
                </Box>
                ...
                <Box sx={{
                    ...
                    <Typography variant="subtitle1" color="text.secondary">Metadata:</Typography>
