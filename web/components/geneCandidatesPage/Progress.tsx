...
export default function Progress({
  ...
  candidate: GeneRow
  candidates_list: GeneRow[]
  currentGeneCandidate: GeneRow | null
  isOpen: boolean
  handleOpen: () => void
  handleExperimentLogsClose: () => void
  handleCandidateChange: (geneId: string) => void
}) {
  ...
  const handleTagsFilter = (tag: string) => {
    if (filteredTags === tag) {
      ...
      setFilteredTags(tag)
      const filtered = progressLogs.filter((log) => log.tags?.some((t) => t.label === tag))
  ...
  const handleSend = async () => {
    ...
    const { data, error } = await supabase.from('experiment_progress_logs').insert([
      {
        ...newGeneLog,
        timestamp:
          typeof newGeneLog.timestamp === 'string'
    ...
    setProgressLogs((prevLogs) => [...prevLogs, newGeneLog])
                  ...
                  <Box sx={{ display: 'flex', alignItems: 'center', mt: 1, ml: 1, gap: 1 }}>
                    ...
                    <Box sx={{ display: 'flex', gap: 1, ml: 'auto' }}>
                      {log.tags?.map((tag: { label: string; color: string }, i: number) => (
                        <Box
                          key={i}
                          onClick={() => handleTagsFilter(tag.label)}
                          sx={{
                            px: 1.2,
                            py: 0.3,
                            borderRadius: 2,
