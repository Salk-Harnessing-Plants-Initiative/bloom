...
export default function ScanTraitBoxplot({
  ...
  useEffect(() => {
    const get = async () => {
      const offsets = traitData.filter((row) => !isNaN(row.trait_value)).map((row) => 0)
      ...
      if (plantId) {
        const scanId_ = traitData.find((row) => row.plant_id === plantId)?.scan_id
        ...
        <svg height={uniqueAccessions.length * ySpacing} width={totalWidth}>
          <g>
            {uniqueAccessions.map((accession, index) => {
              ...
              return (
                <g key={accession} transform={`translate(${xOrigin - labelOffset}, ${yOffset})`}>
                  <text
                    key={accession}
                    style={{
                      fontSize: '12px',
          ...
          <g>
            {traitData
              ...
              .map((row, index) => {
                const accessionIndex = uniqueAccessions?.findIndex((a) => a === row.accession_name)
                return (
                  <circle
                    key={row.scan_id}
                    className={
...
async function getScan(scanId: number, supabase: SupabaseClient<Database, 'public', 'public'>) {
  const { data } = await supabase
    .from('cyl_scans')
    .select(
