...
export default function ExpressionGeneLevelScatterPlot({
  scatterplot,
  show_scatterplot,
  drill_down_gene,
}: {
  scatterplot: Scatterplot
  show_scatterplot: Boolean
  drill_down_gene: String
}) {
    ...
    if (show_scatterplot) {
      ...
      Object.keys(scatterplot).forEach((cluster) => {
        scatterplot[cluster].forEach((point) => {
          points.push({
            ...
            y: point.y,
            cluster: cluster,
            expression_level: point.expression_level,
          })
        })
      })
      ...
      if (scatterplot) {
        ...
        const xScale = d3
          .scaleLinear()
          .domain(
            (d3.extent(points, (d: { x: number; y: number; cluster: string }) => d.x) as [
              number,
              number,
            ]) || [0, 0]
          )
        ...
        const highlight = (event: any, d: Point) => {
          ...
          const doNotHighlight = (event: any, d: Point) => {
            ...
            d3.selectAll(`.${d.cluster}`).transition().duration(200).attr('r', 5)
          }
          ...
          d3.select('#scatter-plot-cluster-id').text('Cell Type (Cluster ID): ' + selectedLabel)
        }
        ...
        const doNotHighlight = (event: any, d: Point) => {
          ...
          d3.select('#scatter-plot-cluster-id').text('')
        }
      ...
      <div>
        <h1 style={{ textAlign: 'center', marginTop: '20px', position: 'relative' }}>
          ...
          <h1
            ...
            style={{ fontWeight: 'bold', position: 'absolute', left: '40%' }}
          >
            ' '
          </h1>
        </h1>
        <svg style={{ width: '100%', height: '100%' }} id="chart-container" ref={chartRef} />
      </div>
    )
  )
}
