...
export default function GeneDrillDown({ geneData }: { geneData: GeneData }) {
  ...
  useEffect(() => {
    const counts_obj = geneData.counts.reduce<{ [key: number]: number }>(
      (acc, item) => {
    ...
    const stats_data = geneData?.data?.reduce<{
      [clusterid: string]: {
        barcodes: string[]
        expression: number[]
        points: { expression: number; barcode: string }[]
      ...
      (
        ...
        if (item.cluster_id !== null) {
          ...
          acc[item.cluster_id]?.points.push({
    ...
    const stats = stats_data
      ? Object.keys(stats_data).map((clusterId: string) => {
          const expression_levels = stats_data[clusterId].expression
          const sorted_exp_val = expression_levels
