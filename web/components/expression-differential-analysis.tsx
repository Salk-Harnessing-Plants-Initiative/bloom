...
export default function DifferentialExpressionAnalysis({ file_id }: { file_id: number }) {
  ...
  useEffect(() => {
    ...
    const top20Genes = transformedData.sort((a, b) => a.p_val_adj - b.p_val_adj).slice(0, 20)
    ...
    plot.append('g').attr('transform', `translate(0, ${innerHeight})`).call(d3.axisBottom(xScale))
    ...
    plot
      ...
      .on('mouseover', (event, d) => {
        tooltip.style('display', 'block').html(`Gene: <strong>${d.gene}</strong>`)
      })
      .on('mousemove', (event) => {
        ...
        <Box sx={{ minWidth: 120 }}>
          <FormControl fullWidth>
            ...
            <Select
              ...
              label="Cluster"
              onChange={(e) => {
                const selected = clusterList.find(
                  (cluster) => cluster.cluster_id === e.target.value
                )
      ...
      {chartData && (
        <>
          <div id="volcano plot" style={{ display: 'flex' }}>
            <div
              style={{ flex: 4, display: 'flex', justifyContent: 'center', alignItems: 'center' }}
            >
