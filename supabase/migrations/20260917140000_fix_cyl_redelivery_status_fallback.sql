-- Fix bloom#875: a re-delivery under a NEW ARGO_WORKFLOW_NAME reports failed even
-- though it is a benign no-op. Change: fix-cyl-redelivery-status-fallback.
--
-- WHY: insert_cyl_result_envelope's no-op branch marks cyl_pipeline_run_scans
...
--   trait/blob data has been correct all along. Confirmed live on staging,
--   2026-09-17: a 3-scan batch with 2 genuine re-deliveries produced
--   done_count=0, failed_count=3 on a Workflow that reported Succeeded.
--
-- WHAT: when the primary (argo_workflow_name, source_id)-keyed UPDATE in the
...
--   the primary UPDATE, so a scan already closed out by
--   fail_cyl_pipeline_run_scans_without_result is never resurrected through the
...
BEGIN
    ...
    IF v_was_noop THEN
        ...
        IF p_argo_workflow_name IS NOT NULL THEN
            ...
            -- bloom#875 fallback: a re-delivery dispatched under a NEW workflow
            -- name always has source_id IS NULL on ITS OWN cyl_pipeline_run_scans
            ...
            -- fallback correctly does nothing (status_update_matched stays
            -- false, unchanged from before this fix).
