...
-- Migration to fix the insert_image_v2_0 function and add unique constraints for phenotypers and cyl_scientists tables

ALTER TABLE phenotypers
DROP CONSTRAINT IF EXISTS phenotypers_email_key;

ALTER TABLE phenotypers
ADD CONSTRAINT phenotypers_email_key UNIQUE (email);

ALTER TABLE cyl_scientists
DROP CONSTRAINT IF EXISTS cyl_scientists_email_key;

ALTER TABLE cyl_scientists
ADD CONSTRAINT cyl_scientists_email_key UNIQUE (email);

CREATE OR REPLACE FUNCTION public.insert_image_v2_0(species_common_name text, experiment text, wave_number integer, germ_day integer, germ_day_color text, plant_age_days integer, date_scanned_ date, device_name text, plant_qr_code text, accession_name text, frame_number_ integer, phenotyper_name text, phenotyper_email text, scientist_name text, scientist_email text)
 RETURNS bigint
 LANGUAGE plpgsql
AS $function$
DECLARE
...
BEGIN
    ...
    -- Upsert the scan
    INSERT INTO cyl_scans (plant_id, scanner_id, phenotyper_id, scientist_id,  date_scanned, plant_age_days) VALUES (plant_id_var, scanner_id_var,phenotyper_id_var,scientist_id_var, date_scanned_, plant_age_days)
