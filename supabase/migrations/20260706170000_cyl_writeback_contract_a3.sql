-- Re-pin the cyl write-back RPC's accepted contract_version to 0.1.0a3, matched
-- prefix-tolerantly. Change: repin-cyl-contract-a3 (#393).
--
-- WHY: the A3-traits ResultEnvelope emitter stamps Provenance.contract_version =
--   "0.1.0a3" (bare, read from the installed sleap-roots-contracts distribution).
--   The deployed RPC (20260630180000) pinned 'v0.1.0a2' and compared byte-exact, so
--   it rejected every real envelope on TWO axes: the version (a2->a3) and the
--   git-tag `v` prefix (the $id/tag namespace) vs the bare PEP 440 package version.
--
-- WHAT: CREATE OR REPLACE the function with the version block pinned to '0.1.0a3'
--   and compared after stripping a single lowercase leading `v` from both sides
--   (regexp_replace(...,'^v','')) over coalesced values. Everything else in the
--   function body is byte-identical to 20260630180000. Forward-only: the original
--   migration is deployed + archived and MUST NOT be edited.
--
-- The a2->a3 contract bump is additive (two optional, nullable Provenance fields
-- that ride inside the opaque cyl_trait_sources.metadata jsonb), so no table/column
-- change and no re-run of the change-E RLS lockdown (DROP POLICY) is needed here.
--
-- Owner + EXECUTE grants are re-asserted verbatim (byte-identical to the deployed
-- set) so the migration is self-contained/idempotent; CREATE OR REPLACE preserves
-- owner/ACL, so this is belt-and-suspenders and does NOT widen access.
--
-- Manual rollback: supabase/rollbacks/20260706170000_cyl_writeback_contract_a3_rollback.sql
