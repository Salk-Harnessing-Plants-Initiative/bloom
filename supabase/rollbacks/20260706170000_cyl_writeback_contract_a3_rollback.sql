-- Rollback for 20260706170000_cyl_writeback_contract_a3.sql
-- Manual break-glass only: this repo applies migrations forward via `supabase db push`
-- (no automated down-runner). Restores the write-back RPC to its pre-a3 body by
-- CREATE OR REPLACE-ing the function verbatim as of 20260630180000 — pinned to
-- 'v0.1.0a2' with strict byte-exact equality (IS DISTINCT FROM, no v-normalization).
--
-- Full-body restore (owner + grants included): a partial one-block rollback would not
-- be catalog-equivalent to the deployed function. No policies are touched (the a3
-- migration changed only the function, not RLS).
--
-- WARNING: applying this rollback re-introduces the ORIGINAL latent defect the a3
-- change fixed — the RPC will again reject every real emitter envelope, because it
-- compares the bare PEP 440 contract_version against the v-prefixed 'v0.1.0a2'. Do
-- NOT treat "rolled back" as a safe steady state for the live write-back path.
