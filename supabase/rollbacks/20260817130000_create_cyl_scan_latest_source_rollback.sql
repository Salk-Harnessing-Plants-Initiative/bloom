-- Manual rollback for 20260817130000_create_cyl_scan_latest_source.sql
...
-- in that order.
--
-- *** ROLLBACK ORDER: apply 20260817150000's rollback, then 20260817140000's, THEN this one. ***
-- refresh_cyl_experiment_trait_counts() (20260817140000) reads cyl_scan_latest_source in its own
...
-- about 20260817150000's dependencies transitively, so the full reverse-chronological order still
...
-- `DROP FUNCTION refresh_cyl_experiment_trait_counts()` without running 20260817140000's own
...
BEGIN
    IF EXISTS (
        ...
        RAISE EXCEPTION 'Roll back 20260817140000 (and 20260817150000, if not already done) before this one -- cyl_experiment_trait_counts and/or refresh_cyl_experiment_trait_counts() still reference cyl_scan_latest_source.';
    END IF;
END;
$$;
