...
export async function getImageMetadata(
  ...
  const { path_format_string, fixed_values, accession_info } = metadataFormat as CylMetadataFormat
  ...
  const pathRegexp = '^' + replaceStrings(path_format_string, fieldRegexps) + '$'
  ...
  const metadata = parsedPaths.map((p) => {
    if (!p.metadata) {
      ...
      console.error(`You may need to update the path_format_string field in ${metadataFile}`)
      process.exit(1)
    }

    // check that plant_qr_code is in accession_info
    if (p.metadata.plant_qr_code && !qr_code_to_accession_name[p.metadata.plant_qr_code]) {
      console.error(
...
export async function summarizeImageMetadata(
  ...
  const scanDates = uniqueDefined(metadata.map((m) => m.date_scanned)).map((s) => s.split('T')[0])
...
function parseFields(obj: { [x: string]: string }): CylImageMetadata {
  return {
    ...
    plant_age_days: obj.plant_age_days ? parseInt(obj.plant_age_days) : undefined,
...
function parsePaths(
  ...
  let parsedPaths = sortedPaths.map((p) => {
    ...
    const finalPath = makeRelativeToCwd ? path.join(relativePrefix, normalizedPath) : normalizedPath
...
export async function uploadImages(
  ...
  opts?: {
    ...
    result?: (index: number, m: CylImageMetadata, created: number | null, error: any) => void
  }
) {
  ...
  await concurrentMap(metadata, nWorkers, async (m, i) => {
    ...
    const { created, error } = await uploadImage(imagePaths[i], metadata[i], uploader, store, {
      pngCompression,
    })
    if (result) {
...
async function uploadImage(
  ...
  if (created !== null && dbError === null) {
    ...
    ;({ error: uploadError } = await uploader.uploadImage(imagePath, targetPath, bucket, {
      pngCompression,
    }))
