...
export async function uploadSCRNAdata(
    ...
    const {created, error} = await store.insertScRna(filename, species_id,scientist_id,url,assembly,annotation,strain,metadata);
...
export async function uploadScRNACounts(
    ...
    const { created, error } = await store.insertScRNAcounts(dataset_id, scrna_counts,store,filename);
    return { created, error };
}