...
export async function createSpecies(
  ...
  for (let { common_name, image_path, genus, species } of speciesList) {
    ...
    if (image_path !== undefined) {
      const illustration_path = path.join(
        "species",
        "species_" + uuid.v4() + ".png"
      );
...
export async function loadSpeciesData(
  ...
  const fs = require("fs");
  const speciesList = yaml.load(
    fs.readFileSync(speciesFile, "utf8")
  ) as SpeciesData[];
  ...
  for (let speciesData of speciesList) {
    if (speciesData.image_path !== undefined) {
      speciesData.image_path = path.join(
        path.dirname(relativePath),
        speciesData.image_path
      );
    }
  }
  return speciesList;
