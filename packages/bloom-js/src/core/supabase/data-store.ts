...
export class SupabaseStore implements DataStore {
  ...
  async insertScRna(dataset_name: string, species_id: number, scientist_id: number, url: string, assembly:string, annotation:string, strain: string, metadata: Json) {
    const { data, error } = await this.supabase
      .from('scrna_datasets')
      .insert([
        {
  ...
  async insertScRNAcells(
    ...
    for (let i = 0; i < scRNA_cells.length; i++) {
      ...
      if (error) {
        console.error(`[ERROR] Failed to insert barcode data into scrna_table. Reason: ${error.message || error}`);
    ...
    for (const key in result) {
      try {
        ...
        if (!gene_id || gene_id.length === 0) {
          console.error(`[ERROR] Reason : Failed to locate gene in gene table while uploading counts.`);
          creationStatus = { created: false, error: null };
          break;
        }
        let clean_filename = filename.trim().replace(/^\\+/, '').replace(/\s+/g, "_").replace(/[<>:"|?*\\%]/g, "").replace(/\.json$/, "");
        ...
        const { error } = await this.supabase.storage.from(bucket_name).upload(storage_path, fileBuffer, { contentType: 'application/json' })
        if (error) {
        ...
        const { data, error: supabase_erro } = await this.supabase.from('scrna_counts').insert([row_data]);
        if (supabase_erro) {
