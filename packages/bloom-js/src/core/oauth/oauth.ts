...
export function encryptToken(text: string): string {
  if (!OAUTH_TOKEN_ENCRYPTION_KEY) throw new Error('OAUTH_TOKEN_ENCRYPTION_KEY is not set')
  const iv = randomBytes(IV_LENGTH)
  const cipher = createCipheriv('aes-256-cbc', Buffer.from(OAUTH_TOKEN_ENCRYPTION_KEY, 'hex'), iv)
