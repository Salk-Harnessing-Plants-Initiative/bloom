...
export type CylScannersRow =
  Database["public"]["Tables"]["cyl_scanners"]["Row"];
...
export interface DataStore {
  ...
  insertScRna(filename: string, species_id: number, scientist_id: number, url: string, assembly:string, annotation:string, strain:string, metadata:Json): Promise<{ error: PostgrestError | string | null, created: number | 0 }>;
  insertScRNAgene(dataset_id: number, scrna_gene_obj: Record<string, any>, store: DataStore): Promise<{ created: boolean, error: PostgrestError | null }>;
  insertScRNAcells(dataset_id: number, scRNA_cells: Record<string, any>, store: DataStore): Promise<{ created: boolean, error: PostgrestError | null }>;
  insertScRNAcounts(dataset_id: number, scRNAcounts: Record<string, any>, store: DataStore, filename: string): Promise<{ created: boolean, error: PostgrestError | null }>
}
