...
# Direct psql access (no host creds needed — runs inside the container):
docker exec -it db-dev psql -U supabase_admin -d postgres
```

> Connecting from the host instead? The password is generated by `make init` and
> the port is `POSTGRES_HOST_PORT` (default 5432) — both live in `.env.dev`, not
> the literal `postgres`/`5432` shown in older examples below.
...
The Supabase CLI tracks applied migrations in
`supabase_migrations.schema_migrations` (the legacy local `_migrations` table is
no longer used). Connection params come from `.env.dev`, so prefer `docker exec`
over a hardcoded host/port/password:

```bash
# Check which migrations are recorded
docker exec db-dev psql -U supabase_admin -d postgres \
    -c "SELECT version FROM supabase_migrations.schema_migrations ORDER BY version;"

# Remove a migration record to re-run it
docker exec db-dev psql -U supabase_admin -d postgres \
    -c "DELETE FROM supabase_migrations.schema_migrations WHERE version = '<YYYYMMDDHHMMSS>';"
